...
from rodan.models import Job, WorkflowJob, ResourceType, Resource
import logging, os
import yaml
        ...
        for mimetype, info in registered_rts.iteritems():
            ...
            if confirm_delete.lower() == 'y':
                try:
                    ...
                    confirm_delete = raw_input("  ..not deleted because of an exception: {0}. Perhaps there are Resources using this ResourceType. Confirm deletion of related Resources (y/N)? ".format(str(e)))
                    if confirm_delete.lower() == 'y':
                        try:
