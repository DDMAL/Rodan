...
class SegmentationTask(Task):
    ...
    def run(self, result_id, runjob_id, *args, **kwargs):
        ...
        if runjob.status == RunJobStatus.RUN_ONCE_WAITING and runjob.needs_input:
            self.retry(args=[result_id, runjob_id], *args, countdown=10, **kwargs)

        elif runjob.status != RunJobStatus.RUN_ONCE_WAITING and runjob.needs_input:
        # This is the first time the job is running. Need to generate the polygons.
