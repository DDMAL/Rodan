...
def define_xml(filename, mime=None):
    ...
    try:
        ...
        if re.search(r"-//Recordare//DTD MusicXML", data[1]) is not None:
            ...
            return "application/x-musicxml+xml"
        ...
        if (
            re.search(r"^\[\[", data[0]) is not None
            and re.search(r"\]\]$", data[0]) is not None
        ):
