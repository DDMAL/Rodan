...
class ResourceDistributor(RodanTask):
    ...
    input_port_types = (
        {'name': 'Resource input', 'minimum': 1, 'maximum': 1, 'resource_types': map(lambda rt: str(rt.mimetype), ResourceType.objects.all())},
    )
    output_port_types = (
        {'name': 'Resource output', 'minimum': 1, 'maximum': 1, 'resource_types': map(lambda rt: str(rt.mimetype), ResourceType.objects.all())},
    )
