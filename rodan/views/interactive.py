...
RODAN_RUNJOB_WORKING_USER_EXPIRY_SECONDS = (
    settings.RODAN_RUNJOB_WORKING_USER_EXPIRY_SECONDS
)
...
class InteractiveAcquireView(generics.GenericAPIView):
    ...
    def post(self, request, run_job_uuid, *args, **kwargs):
        ...
        if runjob.status != task_status.WAITING_FOR_INPUT:
            return Response(
                {"message": "This RunJob does not accept input now"},
        ...
        if (
            ...
            return Response(
                {"message": "Previous working user has not expired yet."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        else:
            ...
            if runjob.working_user_token is None:
                runjob.working_user_token = uuid.uuid4()
            runjob.working_user_expiry = timezone.now() + datetime.timedelta(
                seconds=RODAN_RUNJOB_WORKING_USER_EXPIRY_SECONDS
            )  # extend expiry time
            runjob.save(
                update_fields=[
                    "working_user",
            ...
            return Response(
                {
                    "working_url": request.build_absolute_uri(
                        reverse(
                            "interactive-working",
                            kwargs={
                                "run_job_uuid": str(run_job_uuid),
                                "working_user_token": str(runjob.working_user_token),
                                "additional_url": "",
                            },
                        )
                    ),
...
class InteractiveWorkingView(APIView):
    ...
    def _authenticate(self, run_job_uuid, working_user_token):
        ...
        if runjob.working_user_token != uuid.UUID(working_user_token):
            raise CustomAPIException(
                {"message": "Permission denied"}, status=status.HTTP_401_UNAUTHORIZED
            )
    ...
    def post(self, request, run_job_uuid, working_user_token, additional_url, *a, **k):
        ...
        try:
            setattr(
                manual_task, "url", additional_url
            )  # Set additional information on the object
        ...
        if isinstance(retval, manual_task.WAITING_FOR_INPUT):
            ...
            registry.tasks["rodan.core.master_task"].apply_async(
                (runjob.workflow_run.uuid,)
            )
