    ...
    def perform_create(self, serializer):
        ...
        if auto_expiry_seconds:
            ...
            if not user_set_expiry_time:
                ...
                ):  # [TODO] which users do we allow to create never-expire packages?
                    expiry_time = None
                else:
            ...
            rp = serializer.save(creator=self.request.user, expiry_time=expiry_time)
        else:
            rp = serializer.save(creator=self.request.user, expiry_time=None)
        rp_id = rp.uuid.hex
...
class ResultsPackageDetail(generics.RetrieveDestroyAPIView):
    ...
    def patch(self, request, *args, **kwargs):
        ...
        if (
            ...
            raise CustomAPIException(
                {"status": ["Invalid update"]}, status=status.HTTP_400_BAD_REQUEST
            )

    def perform_destroy(self, instance):
        if instance.status in (task_status.SCHEDULED, task_status.PROCESSING):
            raise CustomAPIException(
                "Please cancel the processing of this package before deleting.",
                status=status.HTTP_400_BAD_REQUEST,
            )
