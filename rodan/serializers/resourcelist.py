...
class ResourceListSerializer(serializers.HyperlinkedModelSerializer):
    ...
    class Meta:
        ...
        fields = ('url', 'uuid', 'name', 'description', 'project', 'resources', 'resource_type', 'origin', 'created', 'updated', 'creator', 'resource_type')
    ...
    def save(self, **kwargs):
        ...
        if resources is not None and len(resources) > 0:
            if project != self.validated_data['resources'][0].project:
                raise serializers.ValidationError("Resources should belong to the same Project as the Resource List.")
