...
class ConnectionViewTestCase(RodanTestTearDownMixin, APITestCase, RodanTestSetUpMixin):
    ...
    def test_post(self):
        ...
        conn_obj = {
            "input_port": "http://testserver/inputport/{0}/".format(
                self.test_inputport.uuid
            ),
            "input_workflow_job": "http://testserver/workflowjob/{0}/".format(
                self.test_workflowjob2.uuid
            ),
            "output_port": "http://testserver/outputport/{0}/".format(
                self.test_outputport.uuid
            ),
            "output_workflow_job": "http://testserver/workflowjob/{0}/".format(
    ...
    def test_delete(self):
        ...
        response = self.client.delete(
            "/connection/{0}/?format=json".format(test_conn_uuid)
        )
    ...
    def test_patch(self):
        ...
        response = self.client.patch(
            "/connection/{0}/".format(conn.uuid), req_obj, format="json"
        )
