...
var h=a(d).attr("pname"),i=parseInt(a(d).attr("oct")),h=new Toe.Model.Custos(h,i);h.setBoundingBox(e);h.setID(a(d).attr("xml:id"));e=new Toe.View.CustosView(f);new Toe.Ctrl.CustosController(h,e);t.setCustos(h);j.debug&&console.log("custos")})});f.repaint()},i=function(b){console.log("loading SVG glyphs ...");return a.get(j.glyphpath,function(c){var d={};a(c).find("svg").each(function(b,c){var e=a("<lol>").append(a(c).clone()).remove().html();fabric.loadSVGFromString(e,function(b){gID=a(c).find("path").attr("id");
