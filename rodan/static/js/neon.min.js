...
b.zone.uly;b=b.zone.lry;c.fixed.push(this.rendEng.createLine([d,g,d,b],e));break;case Toe.Model.Division.Type.div_final:g=b.zone.uly,b=b.zone.lry,c.fixed.push(this.rendEng.createLine([d,g,d,b],e)),d=a.zone.lrx,c.fixed.push(this.rendEng.createLine([d,g,d,b],e))}this.drawing=this.rendEng.draw(c,{group:!0,selectable:a.props.interact,eleRef:a})[0]};Toe.View.DivisionView.prototype.selectDrawing=function(){this.rendEng.canvas.setActiveObject(this.drawing)};Toe.Model.Glyph=function(a,b){this.key=a;this.obj=b;this.centre=[this.obj.width/2,this.obj.height/2]};Toe.Model.Glyph.prototype.constructor=Toe.Glyph;Toe.Model.Glyph.prototype.clone=function(){return this.obj.clone()};Toe.View.GUI=function(a,b,c,d,e){var g={sldr_bgImgOpacity:!0,initBgImgOpacity:0.6,initMode:"edit"};$.extend(g,e);this.rendEng=c;this.page=d;this.prefix=a;this.fileName=b;this.clefDwg=this.divisionDwg=this.punctDwg=null;a=c.getGlyph("punctum").clone();this.punctWidth=a.width*c.getGlobalScale();this.punctHeight=a.height*c.getGlobalScale();this.objMoving=!1;gui=this;this.setupNavBar();this.setupSideBar("#gui-sidebar",g);$("#btn_"+g.initMode).trigger("click")};Toe.View.GUI.prototype.constructor=Toe.View.GUI;
Toe.View.GUI.prototype.setupNavBar=function(){var a=this;$("#nav_file_dropdown").append('<li><a id="nav_file_dropdown_revert" href="#">Revert</a></li><li class="divider"></li><li><a id="nav_file_dropdown_getmei" href="#">Get MEI</a></li><li><a id="nav_file_dropdown_getimg" href="#">Get Score Image</a></li>');$("#nav_file_dropdown_revert").tooltip({animation:!0,placement:"right",title:"<br/><br/>Revert the current MEI file to the original version. Warning: this will revert all changes made in the editor.",
delay:100});$("#nav_file_dropdown_revert").click(function(){$.get(a.prefix+"/revert/"+a.fileName,function(){window.location=a.prefix+"/editor/"+a.fileName}).error(function(){$("#alert > p").text("Server failed to restore backup MEI file.");$("#alert").animate({opacity:1},100)})});$("#nav_file_dropdown_getmei").tooltip({animation:!0,placement:"right",title:"View the MEI file of the document being edited.",delay:100});$("#nav_file_dropdown_getmei").attr("href",this.prefix+"/file/"+this.fileName);$("#nav_file_dropdown_getimg").tooltip({animation:!0,
placement:"right",title:"Download an image of the document being edited.",delay:100});$("#nav_file_dropdown_getimg").click(function(){fabric.Canvas.supports("toDataURL")?window.open(a.rendEng.canvas.toDataURL("png")):$("#alert > p").text("The browser you are using does not support this feature.")})};
...
b.rendEng.canvas.observe("mouse:down",function(a){b.downCoords=b.rendEng.canvas.getPointer(a.memo.e)});b.rendEng.canvas.observe("object:moving",function(){b.objMoving=!0});b.rendEng.canvas.observe("object:selected",function(){var a=b.rendEng.canvas.getActiveObject().eleRef;a instanceof Toe.Model.Neume?($("#info > p").html("Selected: "+a.name+"<br/> Pitche(s): "+$.map(a.components,function(a){return a.pname.toUpperCase()+a.oct}).join(", ")),$("#info").animate({opacity:1},100),$("#menu_editclef").remove(),
...
function(a,b){g.push(b)});$.each(g,function(a,c){var h=c.eleRef;if(h instanceof Toe.Model.Clef){var j=c.left,i=c.top;1<g.length&&(j=e.left+c.left,i=e.top+c.top);j=h.staff.ohSnap({x:j,y:i},null,{ignoreEle:h});j=-Math.round((j.y-h.staff.zone.uly)/(h.staff.delta_y/2));h.setStaffPosition(j);var k=h.staff.getPitchedElements({neumes:!0,custos:!1});0<k.length&&h.staff.getActingClefByEle(k[0])==h&&(j=b.page.getPreviousStaff(h.staff))&&b.handleUpdatePrevCustos(k[0].components[0].pname,k[0].components[0].oct,
j);var j=$.map(h.staff.getPitchedElements({clef:h}),function(a){if(a instanceof Toe.Model.Neume){var c=[];$.each(a.components,function(a,b){c.push({pname:b.pname,oct:b.oct})});return{id:a.id,noteInfo:c}}if(a instanceof Toe.Model.Custos){var e=b.getOutputBoundingBox([a.zone.ulx,a.zone.uly,a.zone.lrx,a.zone.lry]);$.post(b.prefix+"/edit/"+b.fileName+"/move/custos",{id:a.id,ulx:e[0],uly:e[1],lrx:e[2],lry:e[3]}).error(function(){$("#alert > p").text("Server failed to move custos. Client and server are not synchronized.");
$("#alert").animate({opacity:1},100)})}}),i=h.staff.props.numLines+h.props.staffPos/2,k=b.getOutputBoundingBox([h.zone.ulx,h.zone.uly,h.zone.lrx,h.zone.lry]),n={id:h.id,line:i,ulx:k[0],uly:k[1],lrx:k[2],lry:k[3],pitchInfo:j};$.post(b.prefix+"/edit/"+b.fileName+"/move/clef",{data:JSON.stringify(n)}).error(function(){$("#alert > p").text("Server failed to move clef. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)})}else if(h instanceof Toe.Model.Neume){j=c.left;i=c.top;
1<g.length&&(j=e.left+c.left,i=e.top+c.top);var m={x:j,y:h.staff.zone.uly-h.rootStaffPos*h.staff.delta_y/2-d},o=b.page.getClosestStaff(m),p=o.ohSnap(m,c.currentWidth,{ignoreEle:h}),j=Math.round((o.zone.uly-p.y)/(o.delta_y/2)),k=p.x-c.currentWidth/2,i=i-c.currentHeight/2-(m.y-p.y),k=[k,i,k+c.currentWidth,i+c.currentHeight];h.setBoundingBox(k);m=h.rootStaffPos;$.each(h.components,function(a,b){var c=o.calcPitchFromCoords({x:p.x,y:p.y-o.delta_y/2*b.pitchDiff});b.setPitchInfo(c.pname,c.oct)});$(h).trigger("vEraseDrawing");
h.staff.removeElementByRef(h);i=o.addNeume(h);1==g.length&&$(h).trigger("vSelectDrawing");k=b.getOutputBoundingBox([h.zone.ulx,h.zone.uly,h.zone.lrx,h.zone.lry]);n={id:h.id,ulx:k[0],uly:k[1],lrx:k[2],lry:k[3]};m!=j?(n.pitchInfo=[],$.each(h.components,function(a,b){n.pitchInfo.push({pname:b.pname,oct:b.oct})}),h==o.elements[1]&&(j=b.page.getPreviousStaff(o))&&b.handleUpdatePrevCustos(h.components[0].pname,h.components[0].oct,j)):n.pitchInfo=null;i+1<o.elements.length?n.beforeid=o.elements[i+1].id:
(j=b.page.getNextStaff(o),n.beforeid=j.id);$.post(b.prefix+"/edit/"+b.fileName+"/move/neume",{data:JSON.stringify(n)}).error(function(){$("#alert > p").text("Server failed to move neume. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)})}else if(h instanceof Toe.Model.Division){j=c.left;i=c.top;1<g.length&&(j+=e.left,i+=e.top);m={x:j,y:i};j=b.page.getClosestStaff(m);p=j.ohSnap(m,c.currentWidth,{x:!0,y:!1});switch(h.type){case Toe.Model.Division.Type.div_small:p.y=j.zone.uly;
break;case Toe.Model.Division.Type.div_minor:p.y=j.zone.uly+(j.zone.lry-j.zone.uly)/2;break;case Toe.Model.Division.Type.div_major:p.y=j.zone.uly+(j.zone.lry-j.zone.uly)/2;break;case Toe.Model.Division.Type.div_final:p.y=j.zone.uly+(j.zone.lry-j.zone.uly)/2}h.staff.removeElementByRef(h);b.rendEng.canvas.remove(c);b.rendEng.repaint();k=p.x-c.currentWidth/2;i=p.y-c.currentHeight/2;k=[k,i,k+c.currentWidth,i+c.currentHeight];h.setBoundingBox(k);k=j.addDivision(h);1==g.length&&h.selectDrawing();i=null;
k+1<j.elements.length?i=j.elements[k+1].id:(j=b.page.getNextStaff(o),i=j.id);k=b.getOutputBoundingBox([h.zone.ulx,h.zone.uly,h.zone.lrx,h.zone.lry]);$.post(b.prefix+"/edit/"+b.fileName+"/move/division",{id:h.id,ulx:k[0],uly:k[1],lrx:k[2],lry:k[3],beforeid:i}).error(function(){$("#alert > p").text("Server failed to move division. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)})}});1<g.length&&b.rendEng.canvas.discardActiveGroup();b.rendEng.repaint()}b.objMoving=!1}});
$("#btn_delete").unbind("click");$("#btn_neumify").unbind("click");$("#btn_ungroup").unbind("click");$("#btn_delete").bind("click.edit",{gui:b},b.handleDelete);$("#btn_neumify").bind("click.edit",{gui:b},b.handleNeumify);$("#btn_neumify_liquescence").bind("click.edit",{gui:b,modifier:"alt"},b.handleNeumify);$("#btn_ungroup").bind("click.edit",{gui:b},b.handleUngroup)};
Toe.View.GUI.prototype.handleDotToggle=function(a){var b=a.data.gui,c=a.data.punctum;if(a=c.components[0].hasOrnament("dot"))c.components[0].removeOrnament("dot");else{var d=new Toe.Model.Ornament("dot",{form:"aug"});c.components[0].addOrnament(d)}c.syncDrawing();d=b.getOutputBoundingBox([c.zone.ulx,c.zone.uly,c.zone.lrx,c.zone.lry]);c={id:c.id,dotform:"aug",ulx:d[0],uly:d[1],lrx:d[2],lry:d[3]};a?$.post(b.prefix+"/edit/"+b.fileName+"/delete/dot",c).error(function(){$("#alert > p").text("Server failed to remove dot from the punctum. Client and server are not synchronized.");
$("#alert").animate({opacity:1},100)}):$.post(b.prefix+"/edit/"+b.fileName+"/insert/dot",c).error(function(){$("#alert > p").text("Server failed to add a dot to the punctum. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)});$(this).toggleClass("active")};
Toe.View.GUI.prototype.handleHeadShapeChange=function(a){var b=a.data.gui,c=a.data.shape,a=a.data.punctum;a.components[0].setHeadShape(c);"virga"==c?(a.name="Virga",a.typeid="virga"):"cavum"==c&&(a.name="Cavum",a.typeid="cavum");a.syncDrawing();var d=b.getOutputBoundingBox([a.zone.ulx,a.zone.uly,a.zone.lrx,a.zone.lry]);$.post(b.prefix+"/edit/"+b.fileName+"/update/neume/headshape",{id:a.id,shape:c,ulx:d[0],uly:d[1],lrx:d[2],lry:d[3]}).error(function(){$("#alert > p").text("Server failed to change note head shape. Client and server are not synchronized.");
...
oct:b.oct})});return{id:a.id,noteInfo:c}}if(a instanceof Toe.Model.Custos){var e=b.getOutputBoundingBox([a.zone.ulx,a.zone.uly,a.zone.lrx,a.zone.lry]);$.post(b.prefix+"/edit/"+b.fileName+"/move/custos",{id:a.id,ulx:e[0],uly:e[1],lrx:e[2],lry:e[3]}).error(function(){$("#alert > p").text("Server failed to move custos. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)})}});e=b.getOutputBoundingBox([c.zone.ulx,c.zone.uly,c.zone.lrx,c.zone.lry]);$.post(b.prefix+"/edit/"+b.fileName+
"/update/clef/shape",{data:JSON.stringify({id:c.id,shape:a,ulx:e[0],uly:e[1],lrx:e[2],lry:e[3],pitchInfo:d})}).error(function(){$("#alert > p").text("Server failed to update clef shape. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)});$(this).toggleClass("active")}};
Toe.View.GUI.prototype.handleDelete=function(a){var b=a.data.gui;toDelete={clefs:[],nids:[],dids:[]};var c=function(a){var c=a.eleRef,e=c.staff,d=e.getPreviousClef(c),j=e.getPitchedElements(c);e.removeElementByRef(c);e.updatePitchedElements(d);e=$.map(j,function(a){if(a instanceof Toe.Model.Neume){var c=[];$.each(a.components,function(a,b){c.push({pname:b.pname,oct:b.oct})});return{id:a.id,noteInfo:c}}if(a instanceof Toe.Model.Custos){var e=b.getOutputBoundingBox([a.zone.ulx,a.zone.uly,a.zone.lrx,
a.zone.lry]);$.post(b.prefix+"/edit/"+b.fileName+"/move/custos",{id:a.id,ulx:e[0],uly:e[1],lrx:e[2],lry:e[3]}).error(function(){$("#alert > p").text("Server failed to move custos. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)})}});toDelete.clefs.push({id:c.id,pitchInfo:e});b.rendEng.canvas.remove(a);b.rendEng.canvas.discardActiveObject()},d=function(a){var a=a.eleRef,c=a.staff.getPitchedElements({neumes:!0,custos:!1});a.staff.removeElementByRef(a);toDelete.nids.push(a.id);
b.rendEng.canvas.discardActiveObject();if(1==c.length){var e=b.page.getPreviousStaff(a.staff);e&&e.custos&&(e.custos.eraseDrawing(),e.removeElementByRef(e.custos),$.post(b.prefix+"/edit/"+b.fileName+"/delete/custos",{id:e.custos.id}).error(function(){$("#alert > p").text("Server failed to delete custos. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)}),e.custos=null)}else if(a==c[0]&&(e=b.page.getPreviousStaff(a.staff))&&e.custos){var a=e.custos,d=c[1],c=d.components[0].pname,
d=d.components[0].oct,j=e.getActingClefByEle(a),e=e.calcStaffPosFromPitch(c,d,j);a.pname=c;a.oct=d;a.setRootStaffPos(e);e=b.getOutputBoundingBox([a.zone.ulx,a.zone.uly,a.zone.lrx,a.zone.lry]);$.post(b.prefix+"/edit/"+b.fileName+"/move/custos",{id:a.id,pname:c,oct:d,ulx:e[0],uly:e[1],lrx:e[2],lry:e[3]}).error(function(){$("#alert > p").text("Server failed to move custos. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)})}},e=function(a){var e=a.eleRef;e.staff.removeElementByRef(e);
toDelete.dids.push(e.id);b.rendEng.canvas.remove(a);b.rendEng.canvas.discardActiveObject()};if(a=b.rendEng.canvas.getActiveObject())a.eleRef instanceof Toe.Model.Clef&&a.eleRef.staff.elements[0]!=a.eleRef?c(a):a.eleRef instanceof Toe.Model.Neume?d(a):a.eleRef instanceof Toe.Model.Division&&e(a),b.rendEng.repaint();else if(a=b.rendEng.canvas.getActiveGroup())$.each(a.getObjects(),function(a,b){b.eleRef instanceof Toe.Model.Clef&&b.eleRef.staff.elements[0]!=b.eleRef?c(b):b.eleRef instanceof Toe.Model.Neume?
d(b):b.eleRef instanceof Toe.Model.Division&&e(b)}),b.rendEng.canvas.discardActiveGroup(),b.rendEng.repaint();0<toDelete.clefs.length&&$.post(b.prefix+"/edit/"+b.fileName+"/delete/clef",{data:JSON.stringify(toDelete.clefs)}).error(function(){$("#alert > p").text("Server failed to delete clef. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)});0<toDelete.nids.length&&$.post(b.prefix+"/edit/"+b.fileName+"/delete/neume",{ids:toDelete.nids.join(",")}).error(function(){$("#alert > p").text("Server failed to delete neume. Client and server are not synchronized.");
$("#alert").animate({opacity:1},100)});0<toDelete.dids.length&&$.post(b.prefix+"/edit/"+b.fileName+"/delete/division",{ids:toDelete.dids.join(",")}).error(function(){$("#alert > p").text("Server failed to delete division. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)})};
Toe.View.GUI.prototype.handleNeumify=function(a){var b=a.data.gui,a=a.data.modifier,c=b.rendEng.canvas.getActiveGroup();if(c){var d=[],e=null;$.each(c.getObjects(),function(a,b){b.eleRef instanceof Toe.Model.Neume&&(e||(e=b.eleRef.staff),b.eleRef.staff==e&&d.push(b))});if(!(2>d.length)){d.sort(function(a,b){return a.eleRef.zone.ulx-b.eleRef.zone.ulx});var g=new Toe.Model.Neume({modifier:a});numPunct=0;var f=[],l=Number.MAX_VALUE,h=Number.MAX_VALUE,j=Number.MIN_VALUE;$.each(d,function(a,d){$.merge(g.components,
d.eleRef.components);numPunct+=d.eleRef.components.length;f.push(d.eleRef.id);var i=c.top+d.top;l=Math.min(l,c.left+d.left-d.currentHeight/2);h=Math.min(h,i-d.currentHeight/2);j=Math.max(j,i+d.currentHeight/2);e.removeElementByRef(d.eleRef);b.rendEng.canvas.remove(d)});g.setBoundingBox([l,h,l+numPunct*b.punctWidth,j]);a=new Toe.View.NeumeView(b.rendEng);new Toe.Ctrl.NeumeController(g,a);e.addNeume(g);var a=b.getOutputBoundingBox([g.zone.ulx,g.zone.uly,g.zone.lrx,g.zone.lry]),i=g.typeid,k=$.map(g.components,
function(a){return a.props.type}),a=JSON.stringify({nids:f.join(","),typeid:i,headShapes:k,ulx:a[0],uly:a[1],lrx:a[2],lry:a[3]});$.post(b.prefix+"/edit/"+b.fileName+"/neumify",{data:a},function(a){g.id=JSON.parse(a).id}).error(function(){$("#alert > p").text("Server failed to neumify selected neumes. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)});b.rendEng.canvas.discardActiveGroup();$(g).trigger("vSelectDrawing");b.rendEng.repaint()}}};
Toe.View.GUI.prototype.handleUngroup=function(a){var b=a.data.gui,c=[];(a=b.rendEng.canvas.getActiveObject())?a.eleRef instanceof Toe.Model.Neume&&1<a.eleRef.components.length&&c.push(a):(a=b.rendEng.canvas.getActiveGroup())&&$.each(a.getObjects(),function(a,b){b.eleRef instanceof Toe.Model.Neume&&1<b.eleRef.components.length&&c.push(b)});var d=[],e=[],g=[];$.each(c,function(a,c){d.push(c.eleRef.id);var h=[],j=c.eleRef.zone.ulx;c.eleRef.staff.removeElementByRef(c.eleRef);b.rendEng.canvas.remove(c);
$.each(c.eleRef.components,function(a,e){var d=new Toe.Model.Neume;d.components.push(e);var f=c.eleRef.staff.zone.uly-(c.eleRef.rootStaffPos+e.pitchDiff)*c.eleRef.staff.delta_y/2-b.punctHeight/2;d.setBoundingBox([j+a*b.punctWidth,f,j+(a+1)*b.punctWidth,f+b.punctHeight]);f=new Toe.View.NeumeView(b.rendEng);new Toe.Ctrl.NeumeController(d,f);c.eleRef.staff.addNeume(d);f=b.getOutputBoundingBox([d.zone.ulx,d.zone.uly,d.zone.lrx,d.zone.lry]);h.push({ulx:f[0],uly:f[1],lrx:f[2],lry:f[3]});g.push(d)});e.push(h)});
a=JSON.stringify({nids:d.join(","),bbs:e});$.post(b.prefix+"/edit/"+b.fileName+"/ungroup",{data:a},function(a){var b=JSON.parse(a).nids,b=$.map(b,function(a){return a});$.each(g,function(a,c){c.id=b[a]})}).error(function(){$("#alert > p").text("Server failed to ungroup selected neumes. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)});b.rendEng.canvas.discardActiveObject();b.rendEng.canvas.discardActiveGroup();b.rendEng.repaint()};
...
$("#rad_punctum").unbind("click");$("#rad_division").unbind("click");$("#rad_clef").unbind("click");$("#rad_punctum").bind("click.insert",{gui:b},b.handleInsertPunctum);$("#rad_division").bind("click.insert",{gui:b},b.handleInsertDivision);$("#rad_clef").bind("click.insert",{gui:b},b.handleInsertClef);$("#rad_punctum").trigger("click")};
...
function(a){a=b.rendEng.canvas.getPointer(a.memo.e);b.punctDwg.left=a.x-b.punctDwg.currentWidth/4;b.punctDwg.top=a.y-b.punctDwg.currentHeight/4;b.rendEng.repaint()});b.rendEng.canvas.observe("mouse:up",function(){var a={x:b.punctDwg.left,y:b.punctDwg.top},d=b.page.getClosestStaff(a),f=new Toe.Model.Neume,a=d.ohSnap(a,b.punctDwg.currentWidth),l=a.x-b.punctDwg.currentWidth/2,h=a.y-b.punctDwg.currentHeight/2;f.setBoundingBox([l,h,l+b.punctDwg.currentWidth,h+b.punctDwg.currentHeight]);var a=d.calcPitchFromCoords(a),
l=a.pname,h=a.oct,a={pname:l,oct:h},j=[];c&&(j.push(new Toe.Model.Ornament("dot",{form:"aug"})),a.dotform="aug");f.addComponent("punctum",l,h,{ornaments:j});j=new Toe.View.NeumeView(b.rendEng);new Toe.Ctrl.NeumeController(f,j);j=d.addNeume(f);if(1==j){var i=b.page.getPreviousStaff(d);i&&b.handleUpdatePrevCustos(l,h,i)}l=b.getOutputBoundingBox([f.zone.ulx,f.zone.uly,f.zone.lrx,f.zone.lry]);a.ulx=l[0];a.uly=l[1];a.lrx=l[2];a.lry=l[3];if(j+1<d.elements.length)a.beforeid=d.elements[j+1].id;else if(d=
b.page.getNextStaff(d))a.beforeid=d.id;$.post(b.prefix+"/edit/"+b.fileName+"/insert/neume",a,function(a){f.id=JSON.parse(a).id}).error(function(){$("#alert > p").text("Server failed to insert neume. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)})});$("#chk_dot").bind("click.insert",function(){c=c?!1:!0;d(!1)})};
...
var c=null,d=null;b.rendEng.canvas.observe("mouse:move",function(a){a=b.rendEng.canvas.getPointer(a.memo.e);d=b.page.getClosestStaff(a);var g={strokeWidth:4,opacity:0.6};switch(c){case "div_small":a.y=d.zone.uly;if(!b.divisionDwg){var f=d.zone.uly-d.delta_y/2,l=d.zone.uly+d.delta_y/2,h=a.x;b.divisionDwg=b.rendEng.createLine([h,f,h,l],g);b.rendEng.draw({fixed:[b.divisionDwg],modify:[]},{selectable:!1,opacity:0.6})}break;case "div_minor":a.y=d.zone.uly+(d.zone.lry-d.zone.uly)/2;b.divisionDwg||(f=d.zone.uly+
d.delta_y/2,l=f+2*d.delta_y,h=a.x,b.divisionDwg=b.rendEng.createLine([h,f,h,l],g),b.rendEng.draw({fixed:[b.divisionDwg],modify:[]},{selectable:!1,opacity:0.6}));break;case "div_major":a.y=d.zone.uly+(d.zone.lry-d.zone.uly)/2;b.divisionDwg||(f=d.zone.uly,l=d.zone.lry,h=a.x,b.divisionDwg=b.rendEng.createLine([h,f,h,l],g),b.rendEng.draw({fixed:[b.divisionDwg],modify:[]},{selectable:!1,opacity:0.6}));break;case "div_final":if(a.y=d.zone.uly+(d.zone.lry-d.zone.uly)/2,!b.divisionDwg){var f=d.zone.uly,l=
d.zone.lry,h=a.x,j=a.x+b.punctWidth,h=b.rendEng.createLine([h,f,h,l],g),g=b.rendEng.createLine([j,f,j,l],g);b.divisionDwg=b.rendEng.draw({fixed:[h,g],modify:[]},{group:!0,selectable:!1,opacity:0.6})[0]}}g=a.x-b.divisionDwg.currentWidth/2;f=a.x+b.divisionDwg.currentWidth/2;d.elements[0]instanceof Toe.Model.Clef&&g<=d.elements[0].zone.lrx?a.x=d.elements[0].zone.lrx+b.divisionDwg.currentWidth/2+1:g<=d.zone.ulx&&(a.x=d.zone.ulx+b.divisionDwg.currentWidth/2+1);d.custos&&f>=d.custos.zone.ulx?a.x=d.custos.zone.ulx-
...
new Toe.Ctrl.DivisionController(g,f);f=d.addDivision(g);a=b.getOutputBoundingBox(a);a={type:g.key.slice(4),ulx:a[0],uly:a[1],lrx:a[2],lry:a[3]};f+1<d.elements.length?a.beforeid=d.elements[f+1].id:(f=b.page.getNextStaff(d),a.beforeid=f.id);$.post(b.prefix+"/edit/"+b.fileName+"/insert/division",a,function(a){g.id=JSON.parse(a).id}).error(function(){$("#alert > p").text("Server failed to insert division. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)})});$("#rad_small").bind("click.insert",
...
a.zone.lry]);$.post(b.prefix+"/edit/"+b.fileName+"/move/custos",{id:a.id,ulx:d[0],uly:d[1],lrx:d[2],lry:d[3]}).error(function(){$("#alert > p").text("Server failed to move custos. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)})}});$.post(b.prefix+"/edit/"+b.fileName+"/insert/clef",{data:JSON.stringify(a)},function(a){f.id=JSON.parse(a).id}).error(function(){$("#alert > p").text("Server failed to insert clef. Client and server are not synchronized.");$("#alert").animate({opacity:1},
100)})});$("#rad_doh").unbind("click");$("#rad_fah").unbind("click");$("#rad_doh").bind("click.insert",function(){if(!$(this).hasClass("active")){var a={left:-50,top:-50};b.clefDwg&&(b.rendEng.canvas.remove(b.clefDwg),a={left:b.clefDwg.left,top:b.clefDwg.top});a=b.rendEng.getGlyph("c_clef").clone().set($.extend(a,{opacity:0.6}));b.clefDwg=b.rendEng.draw({fixed:[],modify:[a]},{opacity:0.6,selectable:!1,repaint:!0})[0];c="c"}});$("#rad_fah").bind("click.insert",function(){if(!$(this).hasClass("active")){var a=
{left:-50,top:-50};b.clefDwg&&(b.rendEng.canvas.remove(b.clefDwg),a={left:b.clefDwg.left,top:b.clefDwg.top});a=b.rendEng.getGlyph("f_clef").clone().set($.extend(a,{opacity:0.6}));b.clefDwg=b.rendEng.draw({fixed:[],modify:[a]},{opacity:0.6,selectable:!1,repaint:!0})[0];c="f"}});$("#rad_doh").trigger("click")};
Toe.View.GUI.prototype.handleUpdatePrevCustos=function(a,b,c){var d=c.custos;if(d){d.setRootNote(a,b);var e=c.getActingClefByEle(d);d.setRootStaffPos(c.calcStaffPosFromPitch(a,b,e));e=this.getOutputBoundingBox([d.zone.ulx,d.zone.uly,d.zone.lrx,d.zone.lry]);$.post(this.prefix+"/edit/"+this.fileName+"/move/custos",{id:d.id,pname:a,oct:b,ulx:e[0],uly:e[1],lrx:e[2],lry:e[3]}).error(function(){$("#alert > p").text("Server failed to move custos. Client and server are not synchronized.");$("#alert").animate({opacity:1},
100)})}else{var g=new Toe.Model.Custos(a,b),d=c.zone.lrx-gui.punctWidth/2,e=c.zone.uly;g.setBoundingBox([d,e,d+gui.punctWidth,e+gui.punctHeight]);d=new Toe.View.CustosView(gui.rendEng);new Toe.Ctrl.CustosController(g,d);c.setCustos(g);e=this.getOutputBoundingBox([g.zone.ulx,g.zone.uly,g.zone.lrx,g.zone.lry]);a={id:g.id,pname:a,oct:b,ulx:e[0],uly:e[1],lrx:e[2],lry:e[3]};if(c=gui.page.getNextStaff(c))a.beforeid=c.id;$.post(this.prefix+"/edit/"+this.fileName+"/insert/custos",a,function(a){g.id=JSON.parse(a).id}).error(function(){$("#alert > p").text("Server failed to insert custos. Client and server are not synchronized.");
...
Toe.Model.NeumeComponent.prototype.hasOrnament=function(a){return $.grep(this.props.ornaments,function(b){return b.key==a.toLowerCase()}).length};Toe.Model.NeumeComponent.prototype.addOrnament=function(a){if(!(a instanceof Toe.Model.Ornament))throw Error("NeumeComponent: Invalid ornament");this.hasOrnament(a.key)||this.props.ornaments.push(a)};Toe.Model.NeumeComponent.prototype.removeOrnament=function(a){this.props.ornaments=$.grep(this.props.ornaments,function(b){return b.key==a.toLowerCase()},!0)};(function(a){var b=function(b,d){var e=a(b),g,f,l,h,j,i=a.extend({},{width:800,height:1024,autoLoad:!1,debug:!1,filename:"",backgroundImage:"",backgroundImageOpacity:0.6},d);a.extend(i,{canvasid:"neon-canvas"});var k=function(b){function c(d){var e=parseInt(a(d).attr("ulx")),g=parseInt(a(d).attr("uly")),f=parseInt(a(d).attr("lrx")),d=parseInt(a(d).attr("lry"));return a.map([e,g,f,d],function(a){return Math.round(b.scale*a)})}var d=a("clef, sb",g),e=[];a(d).each(function(b,c){a(c).is("sb")&&e.push(b)});
var f=a("neume, sb",g),j=[];a(f).each(function(b,c){a(c).is("sb")&&j.push(b)});var k=a("division, sb",g),l=[];a(k).each(function(b,c){a(c).is("sb")&&l.push(b)});var p=a("custos, sb",g),n=[];a(p).each(function(b,c){a(c).is("sb")&&n.push(b)});a("sb",g).each(function(m,o){i.debug&&console.log("system "+(m+1));var q=a(o).attr("systemref"),q=a(a(g).find("system[xml\\:id="+q+"]")[0]).attr("facs"),q=a(g).find("zone[xml\\:id="+q+"]")[0],q=c(q);i.debug&&h.outlineBoundingBox(q,{fill:"blue"});var r=new Toe.Model.Staff(q);
r.setID(a(o).attr("xml:id"));0==m&&h.calcScaleFromStaff(r,{overwrite:!0});q=new Toe.View.StaffView(h);new Toe.Ctrl.StaffController(r,q);b.addStaff(r);a(d).slice(e[m]+1,e[m+1]).each(function(b,d){var e=a(d).attr("shape"),f=parseInt(a(d).attr("line")),f=2*-(r.props.numLines-f),j=a(d).attr("facs"),j=a(g).find("zone[xml\\:id="+j+"]")[0],j=c(j);i.debug&&h.outlineBoundingBox(j,{fill:"red"});e=new Toe.Model.Clef(e,{staffPos:f});e.setID(a(d).attr("xml:id"));e.setBoundingBox(j);f=new Toe.View.ClefView(h);
new Toe.Ctrl.ClefController(e,f);r.addClef(e);i.debug&&console.log("clef: "+e.name)});a(f).slice(j[m]+1,j[m+1]).each(function(b,d){var e=new Toe.Model.Neume,f=a(g).find("zone[xml\\:id="+a(d).attr("facs")+"]")[0],f=c(f);i.debug&&h.outlineBoundingBox(f,{fill:"green"});e.neumeFromMei(d,f,r);f=new Toe.View.NeumeView(h);new Toe.Ctrl.NeumeController(e,f);r.addNeume(e);i.debug&&console.log("neume: "+e.name)});a(k).slice(l[m]+1,l[m+1]).each(function(b,d){var e=a(g).find("zone[xml\\:id="+a(d).attr("facs")+
"]")[0],f=c(e);i.debug&&h.outlineBoundingBox(f,{fill:"yellow"});var e="div_"+a(d).attr("form"),j=new Toe.Model.Division(e);j.setBoundingBox(f);j.setID(a(d).attr("xml:id"));f=new Toe.View.DivisionView(h);new Toe.Ctrl.DivisionController(j,f);r.addDivision(j);i.debug&&console.log("division: "+e)});a(p).slice(n[m]+1,n[m+1]).each(function(b,d){var e=a(g).find("zone[xml\\:id="+a(d).attr("facs")+"]")[0],e=c(e);i.debug&&h.outlineBoundingBox(e,{fill:"purple"});var f=a(d).attr("pname"),j=parseInt(a(d).attr("oct")),
f=new Toe.Model.Custos(f,j);f.setBoundingBox(e);f.setID(a(d).attr("xml:id"));e=new Toe.View.CustosView(h);new Toe.Ctrl.CustosController(f,e);r.setCustos(f);i.debug&&console.log("custos")})});h.repaint()},n=function(b){console.log("loading SVG glyphs ...");return a.get(i.prefix+"/static/img/neumes_concat.svg",function(c){var d={};a(c).find("svg").each(function(b,c){var e=a("<lol>").append(a(c).clone()).remove().html();fabric.loadSVGFromString(e,function(b){gID=a(c).find("path").attr("id");d[gID]=new Toe.Model.Glyph(gID,
b[0])})});b.setGlyphs(d)})},m=function(b){console.log("loading background image ...");var c=a.Deferred();i.autoLoad&&i.backgroundImage?fabric.Image.fromURL(i.prefix+"/file/"+b,function(a){f=a.width;l=a.height;c.resolve()}):c.resolve();return c.promise()},o=function(b){var c=a.Deferred();i.autoLoad&&i.filename?a.get(i.prefix+"/file/"+b,function(a){console.log("loading MEI file ...");g=a;c.resolve()}):c.resolve();return c.promise()},p=function(){var b=new Toe.Model.Page,c=a("<canvas>").attr("id",i.canvasid),
d=[i.width,i.height];i.autoLoad&&(d=i.backgroundImage?[f,l]:b.calcDimensions(a(g).find("zone")),b.setPageScale(i.width/d[0]));b.setDimensions(Math.round(d[0]),Math.round(d[1]));c.attr("width",b.width);c.attr("height",b.height);c.attr("style","border: 4px black solid;");e.prepend(c);c={renderOnAddition:!1};i.backgroundImage&&a.extend(c,{backgroundImage:i.prefix+"/file/"+i.backgroundImage,backgroundImageOpacity:i.backgroundImageOpacity,backgroundImageStretch:!0});h.setCanvas(new fabric.Canvas(i.canvasid,
c));if(i.debug){var p=a("<div>").attr("id","fps");p.attr("style","color: red; font-size: 200%");e.prepend(p);h.canvas.onFpsUpdate=function(b){a(p).html("FPS: "+b)}}c=new Toe.View.PageView(h);new Toe.Ctrl.PageController(b,c);i.autoLoad&&g&&k(b);new Toe.View.GUI(i.prefix,i.filename,h,b,{sldr_bgImgOpacity:i.backgroundImage,initBgImgOpacity:i.backgroundImageOpacity});console.log("Neon.js ready ("+(new Date-j)+"ms)")};(function(){j=new Date;h=new Toe.View.RenderEngine;a.when(n(h),o(i.filename),m(i.backgroundImage)).then(p,
function(){console.log("Failure to load the mei file, glyphs, or background image")})})()};a.fn.neon=function(c){return this.each(function(){var d=a(this);if(!d.data("neon")){var e=new b(this,c);d.data("neon",e)}})}})(jQuery);Toe.Model.Neume=function(a){this.zone={};this.props={modifier:null,interact:!0};$.extend(this.props,a);this.props.modifier=Toe.Model.Neume.Modifier[this.props.modifier];void 0==this.props.modifier&&(this.props.modifier=null);this.staff=this.id=this.rootStaffPos=this.neumePrefix=this.typeid=this.name=null;this.components=[]};Toe.Model.Neume.prototype.constructor=Toe.Model.Neume;Toe.Model.Neume.Modifier={alt:"liquescence",shift:"variant 2",ctrl:"variant 3",ctrl_shift:"variant 4"};
...
Toe.View.NeumeView.prototype.drawLedgerLines=function(a,b,c,d){var c=0.75*c,e=this,g=[],f=2*(1-d.props.numLines);$.each(a,function(a,h){if(0<h)for(var j=0;j<=h;j+=2){var i=d.zone.uly-j*d.delta_y/2;g.push(e.rendEng.createLine([b[a]-c,i,b[a]+c,i]))}else if(h<f)for(j=f;j>=h;j-=2)i=d.zone.uly-j*d.delta_y/2,g.push(e.rendEng.createLine([b[a]-c,i,b[a]+c,i]))});this.ledgerLines=this.rendEng.draw({fixed:g,modify:[]},{group:!0,selectable:!1})[0]};
...
"diamond";break;case Toe.Model.NeumeComponent.Type.punctum_inclinatum_parvum:f="diamond_small";break;case Toe.Model.NeumeComponent.Type.quilisma:f="quilisma"}e.push(this.rendEng.getGlyph(f))}var l=this.rendEng.getGlyph("dot"),h={fixed:[],modify:[]};switch(a.typeid){case "punctum":g=a.zone.ulx+e[0].centre[0];f=e[0].clone().set({left:g,top:b[0]});h.modify.push(f);if(a.components[0].hasOrnament("dot")){var j=this.bestDotPlacements(c,b,0);h.modify.push(l.clone().set({left:f.left+2*e[0].centre[0],top:j[0]}))}this.drawLedgerLines([a.rootStaffPos],
[g],2*e[0].centre[0],c);break;case "cavum":g=a.zone.ulx+e[0].centre[0];f=e[0].clone().set({left:g,top:b[0]});h.modify.push(f);a.components[0].hasOrnament("dot")&&(j=this.bestDotPlacements(c,b,0),h.modify.push(l.clone().set({left:f.left+2*e[0].centre[0],top:j[0]})));this.drawLedgerLines([a.rootStaffPos],[g],2*e[0].centre[0],c);break;case "virga":g=a.zone.ulx+e[0].centre[0];f=e[0].clone().set({left:g,top:b[0]});h.modify.push(f);a.components[0].hasOrnament("dot")&&(j=this.bestDotPlacements(c,b,0),h.modify.push(l.clone().set({left:f.left+
2*e[0].centre[0],top:j[0]})));var i=f.left+e[0].centre[0]-1,i=this.rendEng.createLine([i,b[0],i,b[0]+1.5*c.delta_y],{strokeWidth:2,interact:!0});h.fixed.push(i);this.drawLedgerLines([a.rootStaffPos],[g],2*e[0].centre[0],c);break;case "clivis":var k=[];k.push(a.zone.ulx+e[0].centre[0]);var n=e[0].clone().set({left:k[0],top:b[0]});h.modify.push(n);i=n.left+e[0].centre[0]-1;i=this.rendEng.createLine([i,b[0],i,b[1]],{strokeWidth:2,interact:!0});h.fixed.push(i);k.push(k[0]+2*e[1].centre[0]);var m=e[1].clone().set({left:k[1],
top:b[1]});h.modify.push(m);$.each(a.components,function(a,f){if(f.hasOrnament("dot")){var g=d.bestDotPlacements(c,b,a);0<g.length&&h.modify.push(l.clone().set({left:m.left+2*e[1].centre[0],top:g[0]}))}});this.drawLedgerLines($.map(a.components,function(b){return a.rootStaffPos+b.pitchDiff}),k,2*e[0].centre[0],c);break;case "cephalicus":k=[];j=this.rendEng.getGlyph("cephalicus");k.push(a.zone.ulx+j.centre[0]);n=j.clone().set({left:k[0],top:b[0]-j.centre[1]/2});h.modify.push(n);f=k[0]-j.centre[0]+
1;i=this.rendEng.createLine([f,b[0],f,a.zone.lry],{strokeWidth:2,interact:!0});h.fixed.push(i);i=n.left+j.centre[0]-2;i=this.rendEng.createLine([i,b[0],i,b[1]],{strokeWidth:2,interact:!0});h.fixed.push(i);f=this.rendEng.getGlyph("liques_up");k.push(k[0]+j.centre[0]-f.centre[0]);m=f.clone().set({left:k[1],top:b[1]-f.centre[1]/2});h.modify.push(m);$.each(a.components,function(a,f){if(f.hasOrnament("dot")){var g=d.bestDotPlacements(c,b,a);0<g.length&&h.modify.push(l.clone().set({left:m.left+2*e[1].centre[0],
top:g[0]}))}});this.drawLedgerLines($.map(a.components,function(b){return a.rootStaffPos+b.pitchDiff}),k,2*e[0].centre[0],c);break;case "climacus.1":case "climacus.2":case "climacus.3":case "climacus.4":k=[];k.push(a.zone.ulx+e[0].centre[0]);j=e[0].centre[0];f=e[0].clone().set({left:k[0],top:b[0]});h.modify.push(f);i=f.left+e[0].centre[0]-1;i=this.rendEng.createLine([i,b[0],i,b[0]+1.5*c.delta_y],{strokeWidth:2,interact:!0});h.fixed.push(i);for(f=1;f<a.components.length;f++)k.push(k[f-1]+2*e[f-1].centre[0]-
1),1==f&&(k[1]+=j),i=e[f].clone().set({left:k[f],top:b[f]}),h.modify.push(i);$.each(a.components,function(a,f){if(f.hasOrnament("dot")){var g=d.bestDotPlacements(c,b,a);0<g.length&&h.modify.push(l.clone().set({left:k[a]+2*e[1].centre[0],top:g[0]}))}});this.drawLedgerLines($.map(a.components,function(b){return a.rootStaffPos+b.pitchDiff}),k,2*e[0].centre[0],c);break;case "torculus":k=[];k.push(a.zone.ulx+e[0].centre[0]);k.push(k[0]+2*e[0].centre[0]-1);k.push(k[1]+2*e[1].centre[0]-1);n=e[0].clone().set({left:k[0],
top:b[0]});h.modify.push(n);i=n.left+e[0].centre[0]-1;i=this.rendEng.createLine([i,b[0],i,b[1]],{strokeWidth:2,interact:!0});h.fixed.push(i);m=e[1].clone().set({left:k[1],top:b[1]});h.modify.push(m);i=m.left+e[1].centre[0]-1;i=this.rendEng.createLine([i,b[1],i,b[2]],{strokeWidth:2,interact:!0});h.fixed.push(i);f=e[2].clone().set({left:k[2],top:b[2]});h.modify.push(f);$.each(a.components,function(a,f){if(f.hasOrnament("dot")){var g=d.bestDotPlacements(c,b,a);0<g.length&&h.modify.push(l.clone().set({left:k[a]+
2*e[1].centre[0],top:g[0]}))}});this.drawLedgerLines($.map(a.components,function(b){return a.rootStaffPos+b.pitchDiff}),k,2*e[0].centre[0],c);break;case "torculus.resupinus.1":case "torculus.resupinus.2":case "torculus.resupinus.3":case "torculus.resupinus.4":k=[];k.push(a.zone.ulx+e[0].centre[0]);n=e[0].clone().set({left:k[0],top:b[0]});h.modify.push(n);j=this.rendEng.getGlyph("porrect_1");g=j.clone().set({left:k[0]+e[0].centre[0]+j.centre[0]-1,top:b[1]+j.centre[1]/2});h.modify.push(g);var f=g.left-
j.centre[0],i=b[0],o=g.top+j.centre[1],i=this.rendEng.createLine([f,b[1],f,i],{strokeWidth:2,interact:!0});h.fixed.push(i);"torculus.resupinus.1"==a.typeid?k.push(g.left+j.centre[0]-e[3].centre[0]):k.push(g.left+j.centre[0]+e[3].centre[0]);f=e[3].clone().set({left:k[1],top:b[3]});h.modify.push(f);i=g.left+j.centre[0]-1;i=this.rendEng.createLine([i,b[3],i,b[2]],{strokeWidth:2,interact:!0});h.fixed.push(i);"torculus.resupinus.2"==a.typeid&&(i=f.left+e[3].centre[0]-1,i=this.rendEng.createLine([i,b[4],
i,b[3]],{strokeWidth:2,interact:!0}),h.fixed.push(i));for(f=4;f<a.components.length;f++)k.push(k[k.length-1]+2*e[f].centre[0]-1),j=e[f].clone().set({left:k[k.length-1],top:b[f]}),h.modify.push(j);$.each(a.components,function(a,f){if(f.hasOrnament("dot")){var g=d.bestDotPlacements(c,b,a);g.length>0&&h.modify.push(l.clone().set({left:k[a]+2*e[1].centre[0],top:g[0]}))}});this.drawLedgerLines($.map(a.components,function(b){return a.rootStaffPos+b.pitchDiff}),k,2*e[0].centre[0],c);break;case "podatus":k=
[];k.push(a.zone.ulx+e[0].centre[0]);n=e[0].clone().set({left:k[0],top:b[0]});h.modify.push(n);k.push(k[0]+2*e[1].centre[0]);m=e[1].clone().set({left:k[1],top:b[1]});h.modify.push(m);$.each(a.components,function(a,f){if(f.hasOrnament("dot")){var g=d.bestDotPlacements(c,b,a);0<g.length&&h.modify.push(l.clone().set({left:n.left+2*e[1].centre[0],top:g[0]}))}});this.drawLedgerLines($.map(a.components,function(b){return a.rootStaffPos+b.pitchDiff}),k,2*e[0].centre[0],c);break;case "epiphonus":k=[];j=this.rendEng.getGlyph("podatus_ep");
k.push(a.zone.ulx+j.centre[0]);n=j.clone().set({left:k[0],top:b[0]});h.modify.push(n);i=n.left+j.centre[0]-2;i=this.rendEng.createLine([i,b[0],i,b[1]],{strokeWidth:2,interact:!0});h.fixed.push(i);f=this.rendEng.getGlyph("liques_down");k.push(k[0]+j.centre[0]-f.centre[0]);m=f.clone().set({left:k[1],top:b[1]+f.centre[1]/2});h.modify.push(m);$.each(a.components,function(a,f){if(f.hasOrnament("dot")){var g=d.bestDotPlacements(c,b,a);0<g.length&&h.modify.push(l.clone().set({left:n.left+2*e[1].centre[0],
top:g[0]}))}});this.drawLedgerLines($.map(a.components,function(b){return a.rootStaffPos+b.pitchDiff}),k,2*e[0].centre[0],c);break;case "porrectus":f=a.components[0].pitchDiff+a.components[1].pitchDiff;j=null;switch(f){case -1:j=this.rendEng.getGlyph("porrect_1");break;case -2:j=this.rendEng.getGlyph("porrect_2");break;case -3:j=this.rendEng.getGlyph("porrect_3");break;case -4:j=this.rendEng.getGlyph("porrect_4");break;default:j=this.rendEng.getGlyph("porrect_4")}g=j.clone().set({left:a.zone.ulx+
j.centre[0],top:b[0]+j.centre[1]/2});h.modify.push(g);f=g.left-j.centre[0]+1;i=a.zone.lry;o=g.top+j.centre[1];a.zone.lry<g.top+o&&(i=o);i=this.rendEng.createLine([f,b[0],f,i],{strokeWidth:2,interact:!0});h.fixed.push(i);k=g.left+j.centre[0]-e[2].centre[0];f=e[2].clone().set({left:k,top:b[2]});i=f.left+e[2].centre[0]-1;i=this.rendEng.createLine([i,b[2],i,b[1]],{strokeWidth:2,interact:!0});h.fixed.push(i);h.modify.push(f);a.components[2].hasOrnament("dot")&&(j=this.bestDotPlacements(c,b,2),0<j.length&&
h.modify.push(l.clone().set({left:f.left+2*e[2].centre[0],top:j[0]})));this.drawLedgerLines([a.rootStaffPos+a.components[2].pitchDiff],[k],2*e[2].centre[0],c);break;case "porrectus.flexus":j=this.rendEng.getGlyph("porrect_1");g=j.clone().set({left:a.zone.ulx+j.centre[0],top:b[0]+j.centre[1]/2});h.modify.push(g);f=g.left-j.centre[0]+1;i=a.zone.lry;o=g.top+j.centre[1];a.zone.lry<g.top+o&&(i=o);i=this.rendEng.createLine([f,b[0],f,i],{strokeWidth:2,interact:!0});h.fixed.push(i);k=[];k.push(g.left+j.centre[0]+
e[2].centre[0]);f=e[2].clone().set({left:k[0],top:b[2]});i=k[0]-e[2].centre[0]-1;i=this.rendEng.createLine([i,b[2],i,b[1]],{strokeWidth:2,interact:!0});h.fixed.push(i);h.modify.push(f);for(f=3;f<a.components.length;f++)k.push(k[k.length-1]+2*e[f].centre[0]-1),j=e[f].clone().set({left:k[k.length-1],top:b[f]}),h.modify.push(j);for(f=2;f<a.components.length;f++)a.components[f].hasOrnament("dot")&&(j=d.bestDotPlacements(c,b,it),0<j.length&&h.modify.push(l.clone().set({left:n.left+2*e[1].centre[0],top:j[0]})));
break;case "scandicus.1":case "scandicus.2":case "scandicus.3":case "scandicus.4":i=a.components.length;o=a.zone.ulx-e[0].centre[0];k=[];for(f=0;f<i-1;f+=2){yoffset=0;1==Math.abs(a.components[f+1].pitchDiff-a.components[f].pitchDiff)&&(yoffset=1);o+=2*e[f].centre[0]-1;k.push(o);j=e[f].clone().set({left:k[f],top:b[f]-e[f].centre[1]/2+yoffset});h.modify.push(j);k.push(k[f]+2*e[f+1].centre[0]);m=e[f+1].clone().set({left:k[f+1],top:b[f+1]-yoffset});h.modify.push(m);for(g=f;g<f+2;g++)a.components[g].hasOrnament("dot")&&
(j=this.bestDotPlacements(c,b,g),0<j.length&&h.modify.push(l.clone().set({left:m.left+2*e[g].centre[0],top:j[0]})))}1==a.components.length%2&&(o+=2*e[i-1].centre[0]-1,k.push(o),f=e[i-1].clone().set({left:o,top:b[i-1]}),h.modify.push(f),a.components[i-1].hasOrnament("dot")&&(j=this.bestDotPlacements(c,b,i-1),0<j.length&&h.modify.push(l.clone().set({left:f.left+2*e[i-1].centre[0],top:j[0]}))));this.drawLedgerLines($.map(a.components,function(b){return a.rootStaffPos+b.pitchDiff}),k,2*e[0].centre[0],
c);break;case "scandicus.flexus.1":k=[];k.push(a.zone.ulx+e[0].centre[0]);n=e[0].clone().set({left:k[0],top:b[0]});h.modify.push(n);i=n.left+e[0].centre[0];i=this.rendEng.createLine([i,b[0],i,b[1]],{strokeWidth:2,interact:!0});h.fixed.push(i);k.push(k[0]+2*e[1].centre[0]);m=e[1].clone().set({left:k[1],top:b[1]});h.modify.push(m);for(g=0;2>g;g++)a.components[g].hasOrnament("dot")&&(j=this.bestDotPlacements(c,b,g),0<j.length&&h.modify.push(l.clone().set({left:k[g]+2*e[g].centre[0],top:j[0]})));k.push(k[1]+
2*e[2].centre[0]);f=e[2].clone().set({left:k[2],top:b[2]});h.modify.push(f);i=k[2]+e[2].centre[0]-1;i=this.rendEng.createLine([i,b[2],i,b[3]],{strokeWidth:2,interact:!0});h.fixed.push(i);k.push(k[2]+2*e[3].centre[0]);f=e[3].clone().set({left:k[3],top:b[3]});h.modify.push(f);for(g=2;g<a.components.length;g++)a.components[g].hasOrnament("dot")&&(j=d.bestDotPlacements(c,b,it),0<j.length&&h.modify.push(l.clone().set({left:m.left+2*e[1].centre[0],top:j[0]})));this.drawLedgerLines($.map(a.components,function(b){return a.rootStaffPos+
b.pitchDiff}),k,2*e[0].centre[0],c)}this.drawing=this.rendEng.draw(h,{group:!0,selectable:a.props.interact,eleRef:a})[0];$(a).trigger("mUpdateBoundingBox",this.drawing)};Toe.View.NeumeView.prototype.render=function(a){this.drawNeume(a)};Toe.View.NeumeView.prototype.updateDrawing=function(a){this.drawing&&this.rendEng.canvas.remove(this.drawing);this.ledgerLines&&(this.rendEng.canvas.remove(this.ledgerLines),this.ledgerLines=null);this.drawNeume(a);this.selectDrawing();this.rendEng.repaint()};
