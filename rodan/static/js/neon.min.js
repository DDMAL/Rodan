...
{gui:c,parentDivId:a},c.handleEdit);$("#btn_insert").bind("click.insert",{gui:c,parentDivId:a},c.handleInsert)};
...
a.pname.toUpperCase()+a.oct),$("#info").animate({opacity:1},100)),$("#menu_editpunctum").remove(),$("#menu_editclef").remove())});b.rendEng.canvas.observe("selection:cleared",function(){$("#info").animate({opacity:0},100);$("#menu_editpunctum").remove();$("#menu_editclef").remove()});b.rendEng.canvas.observe("mouse:up",function(a){var a=b.rendEng.canvas.getPointer(a.memo.e),c=b.downCoords.y-a.y;if(b.objMoving){var g=b.rendEng.canvas.getActiveObject();if(g){var f=[];g.eleRef?f.push(g):$.each(g.objects,
function(a,b){f.push(b)});$.each(f,function(a,d){var h=d.eleRef;if(h instanceof Toe.Model.Clef){var l=d.left,j=d.top;1<f.length&&(l=g.left+d.left,j=g.top+d.top);l=h.staff.ohSnap({x:l,y:j},null,{ignoreEle:h});l=-Math.round((l.y-h.staff.zone.uly)/(h.staff.delta_y/2));h.setStaffPosition(l);var k=h.staff.getPitchedElements({neumes:!0,custos:!1});0<k.length&&h.staff.getActingClefByEle(k[0])==h&&(l=b.page.getPreviousStaff(h.staff))&&b.handleUpdatePrevCustos(k[0].components[0].pname,k[0].components[0].oct,
l);var l=$.map(h.staff.getPitchedElements({clef:h}),function(a){if(a instanceof Toe.Model.Neume){var c=[];$.each(a.components,function(a,b){c.push({pname:b.pname,oct:b.oct})});return{id:a.id,noteInfo:c}}if(a instanceof Toe.Model.Custos){var d=b.getOutputBoundingBox([a.zone.ulx,a.zone.uly,a.zone.lrx,a.zone.lry]);console.log("POST: "+b.prefix+"/edit/"+b.fileName+"/move/custos");$.post(b.prefix+"/edit/"+b.fileName+"/move/custos",{id:a.id,ulx:d[0],uly:d[1],lrx:d[2],lry:d[3],csrfmiddlewaretoken:b.csrf_token}).error(function(){$("#alert > p").text("Server failed to move custos. Client and server are not synchronized.");
$("#alert").animate({opacity:1},100)})}}),j=h.staff.props.numLines+h.props.staffPos/2,k=b.getOutputBoundingBox([h.zone.ulx,h.zone.uly,h.zone.lrx,h.zone.lry]),n={id:h.id,line:j,ulx:k[0],uly:k[1],lrx:k[2],lry:k[3],pitchInfo:l};console.log("POST: "+b.prefix+"/edit/"+b.fileName+"/move/clef");$.post(b.prefix+"/edit/"+b.fileName+"/move/clef",{data:JSON.stringify(n),csrfmiddlewaretoken:b.csrf_token}).error(function(){$("#alert > p").text("Server failed to move clef. Client and server are not synchronized.");
$("#alert").animate({opacity:1},100)})}else if(h instanceof Toe.Model.Neume){l=d.left;j=d.top;1<f.length&&(l=g.left+d.left,j=g.top+d.top);var o={x:l,y:h.staff.zone.uly-h.rootStaffPos*h.staff.delta_y/2-c},p=b.page.getClosestStaff(o),q=p.ohSnap(o,d.currentWidth,{ignoreEle:h}),l=Math.round((p.zone.uly-q.y)/(p.delta_y/2)),k=q.x-d.currentWidth/2,j=j-d.currentHeight/2-(o.y-q.y),k=[k,j,k+d.currentWidth,j+d.currentHeight];h.setBoundingBox(k);o=h.rootStaffPos;$.each(h.components,function(a,b){var d=p.calcPitchFromCoords({x:q.x,
y:q.y-p.delta_y/2*b.pitchDiff});b.setPitchInfo(d.pname,d.oct)});$(h).trigger("vEraseDrawing");h.staff.removeElementByRef(h);j=p.addNeume(h);1==f.length&&$(h).trigger("vSelectDrawing");k=b.getOutputBoundingBox([h.zone.ulx,h.zone.uly,h.zone.lrx,h.zone.lry]);n={id:h.id,ulx:k[0],uly:k[1],lrx:k[2],lry:k[3]};o!=l?(n.pitchInfo=[],$.each(h.components,function(a,b){n.pitchInfo.push({pname:b.pname,oct:b.oct})}),h==p.elements[1]&&(l=b.page.getPreviousStaff(p))&&b.handleUpdatePrevCustos(h.components[0].pname,
h.components[0].oct,l)):n.pitchInfo=null;j+1<p.elements.length?n.beforeid=p.elements[j+1].id:(l=b.page.getNextStaff(p),n.beforeid=l.id);console.log("POST "+b.prefix+"/edit/"+b.fileName+"/move/neume");$.post(b.prefix+"/edit/"+b.fileName+"/move/neume",{data:JSON.stringify(n),csrfmiddlewaretoken:b.csrf_token}).error(function(){$("#alert > p").text("Server failed to move neume. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)})}else if(h instanceof Toe.Model.Division){l=
d.left;j=d.top;1<f.length&&(l+=g.left,j+=g.top);o={x:l,y:j};l=b.page.getClosestStaff(o);q=l.ohSnap(o,d.currentWidth,{x:!0,y:!1});switch(h.type){case Toe.Model.Division.Type.div_small:q.y=l.zone.uly;break;case Toe.Model.Division.Type.div_minor:q.y=l.zone.uly+(l.zone.lry-l.zone.uly)/2;break;case Toe.Model.Division.Type.div_major:q.y=l.zone.uly+(l.zone.lry-l.zone.uly)/2;break;case Toe.Model.Division.Type.div_final:q.y=l.zone.uly+(l.zone.lry-l.zone.uly)/2}h.staff.removeElementByRef(h);b.rendEng.canvas.remove(d);
b.rendEng.repaint();k=q.x-d.currentWidth/2;j=q.y-d.currentHeight/2;k=[k,j,k+d.currentWidth,j+d.currentHeight];h.setBoundingBox(k);k=l.addDivision(h);1==f.length&&h.selectDrawing();j=null;k+1<l.elements.length?j=l.elements[k+1].id:(l=b.page.getNextStaff(p),j=l.id);k=b.getOutputBoundingBox([h.zone.ulx,h.zone.uly,h.zone.lrx,h.zone.lry]);h={id:h.id,ulx:k[0],uly:k[1],lrx:k[2],lry:k[3],beforeid:j,csrfmiddlewaretoken:b.csrf_token};console.log("POST "+b.prefix+"/edit/"+b.fileName+"/move/division");$.post(b.prefix+
"/edit/"+b.fileName+"/move/division",h).error(function(){$("#alert > p").text("Server failed to move division. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)})}});1<f.length&&b.rendEng.canvas.discardActiveGroup();b.rendEng.repaint()}b.objMoving=!1}});$("#btn_delete").unbind("click");$("#btn_neumify").unbind("click");$("#btn_ungroup").unbind("click");$("#btn_delete").bind("click.edit",{gui:b},b.handleDelete);$("#btn_neumify").bind("click.edit",{gui:b},b.handleNeumify);
...
"/edit/"+b.fileName+"/delete/dot"),$.post(b.prefix+"/edit/"+b.fileName+"/delete/dot",c).error(function(){$("#alert > p").text("Server failed to remove dot from the punctum. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)})):(console.log("POST "+b.prefix+"/edit/"+b.fileName+"/insert/dot"),$.post(b.prefix+"/edit/"+b.fileName+"/insert/dot",c).error(function(){$("#alert > p").text("Server failed to add a dot to the punctum. Client and server are not synchronized.");$("#alert").animate({opacity:1},
100)}));$(this).toggleClass("active")};
Toe.View.GUI.prototype.handleHeadShapeChange=function(a){var b=a.data.gui,c=a.data.shape,d=a.data.punctum;a.preventDefault();a.stopPropagation();d.components[0].setHeadShape(c);"virga"==c?(d.name="Virga",d.typeid="virga"):"cavum"==c&&(d.name="Cavum",d.typeid="cavum");d.syncDrawing();a=b.getOutputBoundingBox([d.zone.ulx,d.zone.uly,d.zone.lrx,d.zone.lry]);c={id:d.id,shape:c,ulx:a[0],uly:a[1],lrx:a[2],lry:a[3],csrfmiddlewaretoken:b.csrf_token};console.log("POST "+b.prefix+"/edit/"+b.fileName+"/update/neume/headshape");
$.post(b.prefix+"/edit/"+b.fileName+"/update/neume/headshape",c).error(function(){$("#alert > p").text("Server failed to change note head shape. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)})};
Toe.View.GUI.prototype.handleClefShapeChange=function(a){var b=a.data.gui,c=a.data.clef,d=a.data.shape;a.preventDefault();a.stopPropagation();if(c.shape!=d){c.setShape(d);a=c.staff.getPitchedElements({neumes:!0,custos:!1});if(0<a.length&&c.staff.getActingClefByEle(a[0])==c){var e=b.page.getPreviousStaff(c.staff);e&&b.handleUpdatePrevCustos(a[0].components[0].pname,a[0].components[0].oct,e)}a=$.map(c.staff.getPitchedElements({clef:c}),function(a){if(a instanceof Toe.Model.Neume){var d=[];$.each(a.components,
function(a,b){d.push({pname:b.pname,oct:b.oct})});return{id:a.id,noteInfo:d}}if(a instanceof Toe.Model.Custos){var c=b.getOutputBoundingBox([a.zone.ulx,a.zone.uly,a.zone.lrx,a.zone.lry]);console.log("POST "+b.prefix+"/edit/"+b.fileName+"/move/custos");$.post(b.prefix+"/edit/"+b.fileName+"/move/custos",{id:a.id,ulx:c[0],uly:c[1],lrx:c[2],lry:c[3],csrfmiddlewaretoken:b.csrf_token}).error(function(){$("#alert > p").text("Server failed to move custos. Client and server are not synchronized.");$("#alert").animate({opacity:1},
100)})}});e=b.getOutputBoundingBox([c.zone.ulx,c.zone.uly,c.zone.lrx,c.zone.lry]);c={id:c.id,shape:d,ulx:e[0],uly:e[1],lrx:e[2],lry:e[3],pitchInfo:a,csrfmiddlewaretoken:b.csrf_token};console.log("POST "+b.prefix+"/edit/"+b.fileName+"/update/clef/shape");$.post(b.prefix+"/edit/"+b.fileName+"/update/clef/shape",{data:JSON.stringify(c),csrfmiddlewaretoken:b.csrf_token}).error(function(){$("#alert > p").text("Server failed to update clef shape. Client and server are not synchronized.");$("#alert").animate({opacity:1},
100)});$(this).toggleClass("active")}};
Toe.View.GUI.prototype.handleDelete=function(a){var b=a.data.gui;a.preventDefault();a.stopPropagation();toDelete={clefs:[],nids:[],dids:[]};var c=function(a){var d=a.eleRef,c=d.staff,e=c.getPreviousClef(d),h=c.getPitchedElements(d);c.removeElementByRef(d);c.updatePitchedElements(e);c=$.map(h,function(a){if(a instanceof Toe.Model.Neume){var d=[];$.each(a.components,function(a,b){d.push({pname:b.pname,oct:b.oct})});return{id:a.id,noteInfo:d}}if(a instanceof Toe.Model.Custos){var c=b.getOutputBoundingBox([a.zone.ulx,
a.zone.uly,a.zone.lrx,a.zone.lry]);console.log("POST "+b.prefix+"/edit/"+b.fileName+"/move/custos");$.post(b.prefix+"/edit/"+b.fileName+"/move/custos",{id:a.id,ulx:c[0],uly:c[1],lrx:c[2],lry:c[3],csrfmiddlewaretoken:b.csrf_token}).error(function(){$("#alert > p").text("Server failed to move custos. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)})}});toDelete.clefs.push({id:d.id,pitchInfo:c});b.rendEng.canvas.remove(a);b.rendEng.canvas.discardActiveObject()},d=function(a){var a=
a.eleRef,c=a.staff.getPitchedElements({neumes:!0,custos:!1});a.staff.removeElementByRef(a);toDelete.nids.push(a.id);b.rendEng.canvas.discardActiveObject();if(1==c.length){var d=b.page.getPreviousStaff(a.staff);d&&d.custos&&(d.custos.eraseDrawing(),d.removeElementByRef(d.custos),console.log("POST "+b.prefix+"/edit/"+b.fileName+"/delete/custos"),$.post(b.prefix+"/edit/"+b.fileName+"/delete/custos",{id:d.custos.id,csrfmiddlewaretoken:b.csrf_token}).error(function(){$("#alert > p").text("Server failed to delete custos. Client and server are not synchronized.");
$("#alert").animate({opacity:1},100)}),d.custos=null)}else if(a==c[0]&&(d=b.page.getPreviousStaff(a.staff))&&d.custos){var a=d.custos,e=c[1],c=e.components[0].pname,e=e.components[0].oct,h=d.getActingClefByEle(a),d=d.calcStaffPosFromPitch(c,e,h);a.pname=c;a.oct=e;a.setRootStaffPos(d);d=b.getOutputBoundingBox([a.zone.ulx,a.zone.uly,a.zone.lrx,a.zone.lry]);console.log("POST "+b.prefix+"/edit/"+b.fileName+"/move/custos");$.post(b.prefix+"/edit/"+b.fileName+"/move/custos",{id:a.id,pname:c,oct:e,ulx:d[0],
uly:d[1],lrx:d[2],lry:d[3],csrfmiddlewaretoken:b.csrf_token}).error(function(){$("#alert > p").text("Server failed to move custos. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)})}},e=function(a){var d=a.eleRef;d.staff.removeElementByRef(d);toDelete.dids.push(d.id);b.rendEng.canvas.remove(a);b.rendEng.canvas.discardActiveObject()};if(a=b.rendEng.canvas.getActiveObject())a.eleRef instanceof Toe.Model.Clef&&a.eleRef.staff.elements[0]!=a.eleRef?c(a):a.eleRef instanceof
...
Toe.View.GUI.prototype.handleNeumify=function(a){var b=a.data.gui,c=a.data.modifier;a.preventDefault();a.stopPropagation();var d=b.rendEng.canvas.getActiveGroup();if(d){var e=[],g=null;$.each(d.getObjects(),function(a,b){b.eleRef instanceof Toe.Model.Neume&&(g||(g=b.eleRef.staff),b.eleRef.staff==g&&e.push(b))});if(!(2>e.length)){e.sort(function(a,b){return a.eleRef.zone.ulx-b.eleRef.zone.ulx});var f=new Toe.Model.Neume({modifier:c});numPunct=0;var m=[],i=Number.MAX_VALUE,h=Number.MAX_VALUE,l=Number.MIN_VALUE;
$.each(e,function(a,c){$.merge(f.components,c.eleRef.components);numPunct+=c.eleRef.components.length;m.push(c.eleRef.id);var e=d.top+c.top;i=Math.min(i,d.left+c.left-c.currentHeight/2);h=Math.min(h,e-c.currentHeight/2);l=Math.max(l,e+c.currentHeight/2);g.removeElementByRef(c.eleRef);b.rendEng.canvas.remove(c)});f.setBoundingBox([i,h,i+numPunct*b.punctWidth,l]);a=new Toe.View.NeumeView(b.rendEng);new Toe.Ctrl.NeumeController(f,a);g.addNeume(f);var a=b.getOutputBoundingBox([f.zone.ulx,f.zone.uly,f.zone.lrx,
f.zone.lry]),c=f.typeid,j=$.map(f.components,function(a){return a.props.type}),a=JSON.stringify({nids:m.join(","),typeid:c,headShapes:j,ulx:a[0],uly:a[1],lrx:a[2],lry:a[3]});console.log("POST "+b.prefix+"/edit/"+b.fileName+"/neumify");$.post(b.prefix+"/edit/"+b.fileName+"/neumify",{data:a,csrfmiddlewaretoken:b.csrf_token},function(a){f.id=JSON.parse(a).id}).error(function(){$("#alert > p").text("Server failed to neumify selected neumes. Client and server are not synchronized.");$("#alert").animate({opacity:1},
100)});b.rendEng.canvas.discardActiveGroup();$(f).trigger("vSelectDrawing");b.rendEng.repaint()}}};
Toe.View.GUI.prototype.handleUngroup=function(a){var b=a.data.gui;a.preventDefault();a.stopPropagation();var c=[];(a=b.rendEng.canvas.getActiveObject())?a.eleRef instanceof Toe.Model.Neume&&1<a.eleRef.components.length&&c.push(a):(a=b.rendEng.canvas.getActiveGroup())&&$.each(a.getObjects(),function(a,b){b.eleRef instanceof Toe.Model.Neume&&1<b.eleRef.components.length&&c.push(b)});var d=[],e=[],g=[];$.each(c,function(a,c){d.push(c.eleRef.id);var i=[],h=c.eleRef.zone.ulx;c.eleRef.staff.removeElementByRef(c.eleRef);
b.rendEng.canvas.remove(c);$.each(c.eleRef.components,function(a,d){var e=new Toe.Model.Neume;e.components.push(d);var f=c.eleRef.staff.zone.uly-(c.eleRef.rootStaffPos+d.pitchDiff)*c.eleRef.staff.delta_y/2-b.punctHeight/2;e.setBoundingBox([h+a*b.punctWidth,f,h+(a+1)*b.punctWidth,f+b.punctHeight]);f=new Toe.View.NeumeView(b.rendEng);new Toe.Ctrl.NeumeController(e,f);c.eleRef.staff.addNeume(e);f=b.getOutputBoundingBox([e.zone.ulx,e.zone.uly,e.zone.lrx,e.zone.lry]);i.push({ulx:f[0],uly:f[1],lrx:f[2],
lry:f[3]});g.push(e)});e.push(i)});a=JSON.stringify({nids:d.join(","),bbs:e});console.log("POST "+b.prefix+"/edit/"+b.fileName+"/ungroup");$.post(b.prefix+"/edit/"+b.fileName+"/ungroup",{data:a,csrfmiddlewaretoken:b.csrf_token},function(a){var b=JSON.parse(a).nids,b=$.map(b,function(a){return a});$.each(g,function(a,c){c.id=b[a]})}).error(function(){$("#alert > p").text("Server failed to ungroup selected neumes. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)});b.rendEng.canvas.discardActiveObject();
...
$("#rad_punctum").unbind("click");$("#rad_division").unbind("click");$("#rad_clef").unbind("click");$("#rad_punctum").bind("click.insert",{gui:b},b.handleInsertPunctum);$("#rad_division").bind("click.insert",{gui:b},b.handleInsertDivision);$("#rad_clef").bind("click.insert",{gui:b},b.handleInsertClef);$("#rad_punctum").trigger("click");return!1};
...
function(a){a=b.rendEng.canvas.getPointer(a.memo.e);b.punctDwg.left=a.x-b.punctDwg.currentWidth/4;b.punctDwg.top=a.y-b.punctDwg.currentHeight/4;b.rendEng.repaint()});b.rendEng.canvas.observe("mouse:up",function(){var a={x:b.punctDwg.left,y:b.punctDwg.top},d=b.page.getClosestStaff(a),f=new Toe.Model.Neume,a=d.ohSnap(a,b.punctDwg.currentWidth),m=a.x-b.punctDwg.currentWidth/2,i=a.y-b.punctDwg.currentHeight/2;f.setBoundingBox([m,i,m+b.punctDwg.currentWidth,i+b.punctDwg.currentHeight]);var a=d.calcPitchFromCoords(a),
m=a.pname,i=a.oct,a={pname:m,oct:i,csrfmiddlewaretoken:b.csrf_token},h=[];c&&(h.push(new Toe.Model.Ornament("dot",{form:"aug"})),a.dotform="aug");f.addComponent("punctum",m,i,{ornaments:h});h=new Toe.View.NeumeView(b.rendEng);new Toe.Ctrl.NeumeController(f,h);h=d.addNeume(f);if(1==h){var l=b.page.getPreviousStaff(d);l&&b.handleUpdatePrevCustos(m,i,l)}m=b.getOutputBoundingBox([f.zone.ulx,f.zone.uly,f.zone.lrx,f.zone.lry]);a.ulx=m[0];a.uly=m[1];a.lrx=m[2];a.lry=m[3];if(h+1<d.elements.length)a.beforeid=
d.elements[h+1].id;else if(d=b.page.getNextStaff(d))a.beforeid=d.id;console.log("POST "+b.prefix+"/edit/"+b.fileName+"/insert/neume");$.post(b.prefix+"/edit/"+b.fileName+"/insert/neume",a,function(a){f.id=JSON.parse(a).id}).error(function(){$("#alert > p").text("Server failed to insert neume. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)})});$("#chk_dot").bind("click.insert",function(){c=c?!1:!0;d(!1)})};
...
var c=null,d=null;b.rendEng.canvas.observe("mouse:move",function(a){a=b.rendEng.canvas.getPointer(a.memo.e);d=b.page.getClosestStaff(a);var g={strokeWidth:4,opacity:0.6};switch(c){case "div_small":a.y=d.zone.uly;if(!b.divisionDwg){var f=d.zone.uly-d.delta_y/2,m=d.zone.uly+d.delta_y/2,i=a.x;b.divisionDwg=b.rendEng.createLine([i,f,i,m],g);b.rendEng.draw({fixed:[b.divisionDwg],modify:[]},{selectable:!1,opacity:0.6})}break;case "div_minor":a.y=d.zone.uly+(d.zone.lry-d.zone.uly)/2;b.divisionDwg||(f=d.zone.uly+
d.delta_y/2,m=f+2*d.delta_y,i=a.x,b.divisionDwg=b.rendEng.createLine([i,f,i,m],g),b.rendEng.draw({fixed:[b.divisionDwg],modify:[]},{selectable:!1,opacity:0.6}));break;case "div_major":a.y=d.zone.uly+(d.zone.lry-d.zone.uly)/2;b.divisionDwg||(f=d.zone.uly,m=d.zone.lry,i=a.x,b.divisionDwg=b.rendEng.createLine([i,f,i,m],g),b.rendEng.draw({fixed:[b.divisionDwg],modify:[]},{selectable:!1,opacity:0.6}));break;case "div_final":if(a.y=d.zone.uly+(d.zone.lry-d.zone.uly)/2,!b.divisionDwg){var f=d.zone.uly,m=
d.zone.lry,i=a.x,h=a.x+b.punctWidth,i=b.rendEng.createLine([i,f,i,m],g),g=b.rendEng.createLine([h,f,h,m],g);b.divisionDwg=b.rendEng.draw({fixed:[i,g],modify:[]},{group:!0,selectable:!1,opacity:0.6})[0]}}g=a.x-b.divisionDwg.currentWidth/2;f=a.x+b.divisionDwg.currentWidth/2;d.elements[0]instanceof Toe.Model.Clef&&g<=d.elements[0].zone.lrx?a.x=d.elements[0].zone.lrx+b.divisionDwg.currentWidth/2+1:g<=d.zone.ulx&&(a.x=d.zone.ulx+b.divisionDwg.currentWidth/2+1);d.custos&&f>=d.custos.zone.ulx?a.x=d.custos.zone.ulx-
...
Toe.Model.NeumeComponent.prototype.hasOrnament=function(a){return $.grep(this.props.ornaments,function(b){return b.key==a.toLowerCase()}).length};Toe.Model.NeumeComponent.prototype.addOrnament=function(a){if(!(a instanceof Toe.Model.Ornament))throw Error("NeumeComponent: Invalid ornament");this.hasOrnament(a.key)||this.props.ornaments.push(a)};Toe.Model.NeumeComponent.prototype.removeOrnament=function(a){this.props.ornaments=$.grep(this.props.ornaments,function(b){return b.key==a.toLowerCase()},!0)};(function(a){var b=function(b,d){var e=a(b),g,f,m,i=a.extend({},{autoLoad:!0,debug:!1,prefix:"neon",pageid:"",meipath:"",bgimgpath:"",glyphpath:"",width:1E3,orig_width:1E3,orig_height:1E3,height:1E3,backgroundImageOpacity:0},d);a.extend(i,{canvasid:"neon-canvas"});var h=function(b){function c(d){var e=parseInt(a(d).attr("ulx")),f=parseInt(a(d).attr("uly")),g=parseInt(a(d).attr("lrx")),d=parseInt(a(d).attr("lry"));return a.map([e,f,g,d],function(a){return Math.round(b.scale*a)})}var d=a("clef, sb",
g),e=[];a(d).each(function(b,c){a(c).is("sb")&&e.push(b)});var h=a("neume, sb",g),j=[];a(h).each(function(b,c){a(c).is("sb")&&j.push(b)});var k=a("division, sb",g),l=[];a(k).each(function(b,c){a(c).is("sb")&&l.push(b)});var m=a("custos, sb",g),u=[];a(m).each(function(b,c){a(c).is("sb")&&u.push(b)});a("sb",g).each(function(r,v){i.debug&&console.log("system "+(r+1));var s=a(v).attr("systemref"),s=a(a(g).find("system[xml\\:id="+s+"]")[0]).attr("facs"),s=a(g).find("zone[xml\\:id="+s+"]")[0],s=c(s);i.debug&&
f.outlineBoundingBox(s,{fill:"blue"});var t=new Toe.Model.Staff(s);t.setID(a(v).attr("xml:id"));0==r&&f.calcScaleFromStaff(t,{overwrite:!0});s=new Toe.View.StaffView(f);new Toe.Ctrl.StaffController(t,s);b.addStaff(t);a(d).slice(e[r]+1,e[r+1]).each(function(b,d){var e=a(d).attr("shape"),h=parseInt(a(d).attr("line")),h=2*-(t.props.numLines-h),j=a(d).attr("facs"),j=a(g).find("zone[xml\\:id="+j+"]")[0],j=c(j);i.debug&&f.outlineBoundingBox(j,{fill:"red"});e=new Toe.Model.Clef(e,{staffPos:h});e.setID(a(d).attr("xml:id"));
e.setBoundingBox(j);h=new Toe.View.ClefView(f);new Toe.Ctrl.ClefController(e,h);t.addClef(e);i.debug&&console.log("clef: "+e.name)});a(h).slice(j[r]+1,j[r+1]).each(function(b,d){var e=new Toe.Model.Neume,h=a(g).find("zone[xml\\:id="+a(d).attr("facs")+"]")[0],h=c(h);i.debug&&f.outlineBoundingBox(h,{fill:"green"});e.neumeFromMei(d,h,t);h=new Toe.View.NeumeView(f);new Toe.Ctrl.NeumeController(e,h);t.addNeume(e);i.debug&&console.log("neume: "+e.name)});a(k).slice(l[r]+1,l[r+1]).each(function(b,d){var e=
a(g).find("zone[xml\\:id="+a(d).attr("facs")+"]")[0],h=c(e);i.debug&&f.outlineBoundingBox(h,{fill:"yellow"});var e="div_"+a(d).attr("form"),j=new Toe.Model.Division(e);j.setBoundingBox(h);j.setID(a(d).attr("xml:id"));h=new Toe.View.DivisionView(f);new Toe.Ctrl.DivisionController(j,h);t.addDivision(j);i.debug&&console.log("division: "+e)});a(m).slice(u[r]+1,u[r+1]).each(function(b,d){var e=a(g).find("zone[xml\\:id="+a(d).attr("facs")+"]")[0],e=c(e);i.debug&&f.outlineBoundingBox(e,{fill:"purple"});
var h=a(d).attr("pname"),j=parseInt(a(d).attr("oct")),h=new Toe.Model.Custos(h,j);h.setBoundingBox(e);h.setID(a(d).attr("xml:id"));e=new Toe.View.CustosView(f);new Toe.Ctrl.CustosController(h,e);t.setCustos(h);i.debug&&console.log("custos")})});f.repaint()},l=function(b){console.log("loading SVG glyphs ...");return a.get(i.glyphpath,function(c){var d={};a(c).find("svg").each(function(b,c){var e=a("<lol>").append(a(c).clone()).remove().html();fabric.loadSVGFromString(e,function(b){gID=a(c).find("path").attr("id");
d[gID]=new Toe.Model.Glyph(gID,b[0])})});b.setGlyphs(d)})},j=function(){var b=a.Deferred();i.autoLoad&&i.meipath?a.get(i.meipath,function(a){console.log("loading MEI file ...");g=a;b.resolve()}):b.resolve();return b.promise()},k=function(){var b=new Toe.Model.Page,c=a("<canvas>").attr("id",i.canvasid),d=[i.width,i.height];i.autoLoad&&(d=i.bgimgpath?[i.orig_width,i.orig_height]:b.calcDimensions(a(g).find("zone")),b.setPageScale(i.width/i.orig_width),console.log("page scale: "+b.scale));b.setDimensions(Math.round(d[0]),
Math.round(d[1]));c.attr("width",b.width);c.attr("height",b.height);c.attr("style","border: 4px black solid;");e.prepend(c);c={renderOnAddition:!1};i.bgimgpath&&a.extend(c,{backgroundImage:i.bgimgpath,backgroundImageOpacity:i.backgroundImageOpacity,backgroundImageStretch:!1});f.setCanvas(new fabric.Canvas(i.canvasid,c));if(i.debug){var j=a("<div>").attr("id","fps");j.attr("style","color: red; font-size: 200%");e.prepend(j);f.canvas.onFpsUpdate=function(b){a(j).html("FPS: "+b)}}c=new Toe.View.PageView(f);
new Toe.Ctrl.PageController(b,c);i.autoLoad&&g&&h(b);new Toe.View.GUI(i.prefix,i.pageid,f,b,i.csrf_token,{sldr_bgImgOpacity:i.bgimgpath,initBgImgOpacity:i.backgroundImageOpacity});console.log("Neon.js ready ("+(new Date-m)+"ms)")};(function(){m=new Date;f=new Toe.View.RenderEngine;a.when(l(f),j()).then(k,function(){console.log("Failure to load the mei file, glyphs, or background image")})})()};a.fn.neon=function(c){return this.each(function(){var d=a(this);if(!d.data("neon")){var e=new b(this,c);
...
Toe.View.NeumeView.prototype.drawLedgerLines=function(a,b,c,d){var c=0.75*c,e=this,g=[],f=2*(1-d.props.numLines);$.each(a,function(a,i){if(0<i)for(var h=0;h<=i;h+=2){var l=d.zone.uly-h*d.delta_y/2;g.push(e.rendEng.createLine([b[a]-c,l,b[a]+c,l]))}else if(i<f)for(h=f;h>=i;h-=2)l=d.zone.uly-h*d.delta_y/2,g.push(e.rendEng.createLine([b[a]-c,l,b[a]+c,l]))});this.ledgerLines=this.rendEng.draw({fixed:g,modify:[]},{group:!0,selectable:!1})[0]};
...
"diamond";break;case Toe.Model.NeumeComponent.Type.punctum_inclinatum_parvum:f="diamond_small";break;case Toe.Model.NeumeComponent.Type.quilisma:f="quilisma"}e.push(this.rendEng.getGlyph(f))}var m=this.rendEng.getGlyph("dot"),i={fixed:[],modify:[]};switch(a.typeid){case "punctum":var h=a.zone.ulx+e[0].centre[0],f=e[0].clone().set({left:h,top:b[0]});i.modify.push(f);if(a.components[0].hasOrnament("dot")){var l=this.bestDotPlacements(c,b,0);i.modify.push(m.clone().set({left:f.left+2*e[0].centre[0],
top:l[0]}))}this.drawLedgerLines([a.rootStaffPos],[h],2*e[0].centre[0],c);break;case "cavum":h=a.zone.ulx+e[0].centre[0];f=e[0].clone().set({left:h,top:b[0]});i.modify.push(f);a.components[0].hasOrnament("dot")&&(l=this.bestDotPlacements(c,b,0),i.modify.push(m.clone().set({left:f.left+2*e[0].centre[0],top:l[0]})));this.drawLedgerLines([a.rootStaffPos],[h],2*e[0].centre[0],c);break;case "virga":h=a.zone.ulx+e[0].centre[0];f=e[0].clone().set({left:h,top:b[0]});i.modify.push(f);a.components[0].hasOrnament("dot")&&
(l=this.bestDotPlacements(c,b,0),i.modify.push(m.clone().set({left:f.left+2*e[0].centre[0],top:l[0]})));var j=f.left+e[0].centre[0]-1,j=this.rendEng.createLine([j,b[0],j,b[0]+1.5*c.delta_y],{strokeWidth:2,interact:!0});i.fixed.push(j);this.drawLedgerLines([a.rootStaffPos],[h],2*e[0].centre[0],c);break;case "clivis":var k=[];k.push(a.zone.ulx+e[0].centre[0]);var n=e[0].clone().set({left:k[0],top:b[0]});i.modify.push(n);f=k[0]-e[0].centre[0]+1;j=this.rendEng.createLine([f,b[0],f,a.zone.lry],{strokeWidth:2,
interact:!0});i.fixed.push(j);j=n.left+e[0].centre[0]-1;j=this.rendEng.createLine([j,b[0],j,b[1]],{strokeWidth:2,interact:!0});i.fixed.push(j);k.push(k[0]+2*e[1].centre[0]);var o=e[1].clone().set({left:k[1],top:b[1]});i.modify.push(o);$.each(a.components,function(a,f){if(f.hasOrnament("dot")){var g=d.bestDotPlacements(c,b,a);0<g.length&&i.modify.push(m.clone().set({left:o.left+2*e[1].centre[0],top:g[0]}))}});this.drawLedgerLines($.map(a.components,function(b){return a.rootStaffPos+b.pitchDiff}),k,
2*e[0].centre[0],c);break;case "cephalicus":k=[];h=this.rendEng.getGlyph("cephalicus");k.push(a.zone.ulx+h.centre[0]);n=h.clone().set({left:k[0],top:b[0]-h.centre[1]/2});i.modify.push(n);f=k[0]-h.centre[0]+1;j=this.rendEng.createLine([f,b[0],f,a.zone.lry],{strokeWidth:2,interact:!0});i.fixed.push(j);j=n.left+h.centre[0]-2;j=this.rendEng.createLine([j,b[0],j,b[1]],{strokeWidth:2,interact:!0});i.fixed.push(j);f=this.rendEng.getGlyph("liques_up");k.push(k[0]+h.centre[0]-f.centre[0]);o=f.clone().set({left:k[1],
top:b[1]-f.centre[1]/2});i.modify.push(o);$.each(a.components,function(a,f){if(f.hasOrnament("dot")){var g=d.bestDotPlacements(c,b,a);0<g.length&&i.modify.push(m.clone().set({left:o.left+2*e[1].centre[0],top:g[0]}))}});this.drawLedgerLines($.map(a.components,function(b){return a.rootStaffPos+b.pitchDiff}),k,2*e[0].centre[0],c);break;case "climacus.1":case "climacus.2":case "climacus.3":case "climacus.4":k=[];k.push(a.zone.ulx+e[0].centre[0]);l=e[0].centre[0];f=e[0].clone().set({left:k[0],top:b[0]});
i.modify.push(f);j=f.left+e[0].centre[0]-1;j=this.rendEng.createLine([j,b[0],j,b[0]+1.5*c.delta_y],{strokeWidth:2,interact:!0});i.fixed.push(j);for(f=1;f<a.components.length;f++)k.push(k[f-1]+2*e[f-1].centre[0]-1),1==f&&(k[1]+=l),j=e[f].clone().set({left:k[f],top:b[f]}),i.modify.push(j);$.each(a.components,function(a,f){if(f.hasOrnament("dot")){var g=d.bestDotPlacements(c,b,a);0<g.length&&i.modify.push(m.clone().set({left:k[a]+2*e[1].centre[0],top:g[0]}))}});this.drawLedgerLines($.map(a.components,
function(b){return a.rootStaffPos+b.pitchDiff}),k,2*e[0].centre[0],c);break;case "torculus":k=[];k.push(a.zone.ulx+e[0].centre[0]);k.push(k[0]+2*e[0].centre[0]-1);k.push(k[1]+2*e[1].centre[0]-1);n=e[0].clone().set({left:k[0],top:b[0]});i.modify.push(n);j=n.left+e[0].centre[0]-1;j=this.rendEng.createLine([j,b[0],j,b[1]],{strokeWidth:2,interact:!0});i.fixed.push(j);o=e[1].clone().set({left:k[1],top:b[1]});i.modify.push(o);j=o.left+e[1].centre[0]-1;j=this.rendEng.createLine([j,b[1],j,b[2]],{strokeWidth:2,
interact:!0});i.fixed.push(j);f=e[2].clone().set({left:k[2],top:b[2]});i.modify.push(f);$.each(a.components,function(a,f){if(f.hasOrnament("dot")){var g=d.bestDotPlacements(c,b,a);0<g.length&&i.modify.push(m.clone().set({left:k[a]+2*e[1].centre[0],top:g[0]}))}});this.drawLedgerLines($.map(a.components,function(b){return a.rootStaffPos+b.pitchDiff}),k,2*e[0].centre[0],c);break;case "torculus.resupinus.1":case "torculus.resupinus.2":case "torculus.resupinus.3":case "torculus.resupinus.4":k=[];k.push(a.zone.ulx+
e[0].centre[0]);n=e[0].clone().set({left:k[0],top:b[0]});i.modify.push(n);l=this.rendEng.getGlyph("porrect_1");h=l.clone().set({left:k[0]+e[0].centre[0]+l.centre[0]-1,top:b[1]+l.centre[1]/2});i.modify.push(h);var f=h.left-l.centre[0],j=b[0],p=h.top+l.centre[1],j=this.rendEng.createLine([f,b[1],f,j],{strokeWidth:2,interact:!0});i.fixed.push(j);"torculus.resupinus.1"==a.typeid?k.push(h.left+l.centre[0]-e[3].centre[0]):k.push(h.left+l.centre[0]+e[3].centre[0]);f=e[3].clone().set({left:k[1],top:b[3]});
i.modify.push(f);j=h.left+l.centre[0]-1;j=this.rendEng.createLine([j,b[3],j,b[2]],{strokeWidth:2,interact:!0});i.fixed.push(j);"torculus.resupinus.2"==a.typeid&&(j=f.left+e[3].centre[0]-1,j=this.rendEng.createLine([j,b[4],j,b[3]],{strokeWidth:2,interact:!0}),i.fixed.push(j));for(f=4;f<a.components.length;f++)k.push(k[k.length-1]+2*e[f].centre[0]-1),j=e[f].clone().set({left:k[k.length-1],top:b[f]}),i.modify.push(j);$.each(a.components,function(a,f){if(f.hasOrnament("dot")){var g=d.bestDotPlacements(c,
b,a);g.length>0&&i.modify.push(m.clone().set({left:k[a]+2*e[1].centre[0],top:g[0]}))}});this.drawLedgerLines($.map(a.components,function(b){return a.rootStaffPos+b.pitchDiff}),k,2*e[0].centre[0],c);break;case "podatus":k=[];l=0;1==Math.abs(a.components[1].pitchDiff)&&(l=1);h=this.rendEng.getGlyph("pes");k.push(a.zone.ulx+h.centre[0]);n=h.clone().set({left:k[0],top:b[0]-h.centre[1]/2+l});i.modify.push(n);j=n.left+h.centre[0]-1;j=this.rendEng.createLine([j,b[0],j,b[1]],{strokeWidth:2,interact:!0});
i.fixed.push(j);k.push(k[0]);o=e[1].clone().set({left:k[1],top:b[1]-l});i.modify.push(o);$.each(a.components,function(a,f){if(f.hasOrnament("dot")){var g=d.bestDotPlacements(c,b,a);0<g.length&&i.modify.push(m.clone().set({left:n.left+2*e[1].centre[0],top:g[0]}))}});this.drawLedgerLines($.map(a.components,function(b){return a.rootStaffPos+b.pitchDiff}),k,2*e[0].centre[0],c);break;case "epiphonus":k=[];h=this.rendEng.getGlyph("podatus_ep");k.push(a.zone.ulx+h.centre[0]);n=h.clone().set({left:k[0],top:b[0]});
i.modify.push(n);j=n.left+h.centre[0]-2;j=this.rendEng.createLine([j,b[0],j,b[1]],{strokeWidth:2,interact:!0});i.fixed.push(j);f=this.rendEng.getGlyph("liques_down");k.push(k[0]+h.centre[0]-f.centre[0]);o=f.clone().set({left:k[1],top:b[1]+f.centre[1]/2});i.modify.push(o);$.each(a.components,function(a,f){if(f.hasOrnament("dot")){var g=d.bestDotPlacements(c,b,a);0<g.length&&i.modify.push(m.clone().set({left:n.left+2*e[1].centre[0],top:g[0]}))}});this.drawLedgerLines($.map(a.components,function(b){return a.rootStaffPos+
b.pitchDiff}),k,2*e[0].centre[0],c);break;case "porrectus":l=this.rendEng.getGlyph("porrect_1");h=l.clone().set({left:a.zone.ulx+l.centre[0],top:b[0]+l.centre[1]/2});i.modify.push(h);f=h.left-l.centre[0]+1;j=a.zone.lry;p=h.top+l.centre[1];a.zone.lry<h.top+p&&(j=p);j=this.rendEng.createLine([f,b[0],f,j],{strokeWidth:2,interact:!0});i.fixed.push(j);k=h.left+l.centre[0]-e[2].centre[0];f=e[2].clone().set({left:k,top:b[2]});j=f.left+e[2].centre[0]-1;j=this.rendEng.createLine([j,b[2],j,b[1]],{strokeWidth:2,
interact:!0});i.fixed.push(j);i.modify.push(f);a.components[2].hasOrnament("dot")&&(l=this.bestDotPlacements(c,b,2),0<l.length&&i.modify.push(m.clone().set({left:f.left+2*e[2].centre[0],top:l[0]})));this.drawLedgerLines([a.rootStaffPos+a.components[2].pitchDiff],[k],2*e[2].centre[0],c);break;case "scandicus.1":case "scandicus.2":case "scandicus.3":case "scandicus.4":j=a.components.length;p=this.rendEng.getGlyph("pes");h=a.zone.ulx-e[0].centre[0];k=[];for(f=0;f<j-1;f+=2){l=0;1==Math.abs(a.components[f+
1].pitchDiff-a.components[f].pitchDiff)&&(l=1);h+=2*p.centre[0]-1;k.push(h);g=p.clone().set({left:h,top:b[f]-p.centre[1]/2+l});i.modify.push(g);g=h+p.centre[0]-1;g=this.rendEng.createLine([g,b[f],g,b[f+1]],{strokeWidth:2,interact:!0});i.fixed.push(g);k.push(h);o=e[f+1].clone().set({left:h,top:b[f+1]-l});i.modify.push(o);for(g=f;g<f+2;g++)a.components[g].hasOrnament("dot")&&(l=this.bestDotPlacements(c,b,g),0<l.length&&i.modify.push(m.clone().set({left:o.left+2*e[g].centre[0],top:l[0]})))}1==a.components.length%
2&&(h+=2*e[j-1].centre[0]-1,k.push(h),f=e[j-1].clone().set({left:h,top:b[j-1]}),i.modify.push(f),a.components[j-1].hasOrnament("dot")&&(l=this.bestDotPlacements(c,b,j-1),0<l.length&&i.modify.push(m.clone().set({left:f.left+2*e[j-1].centre[0],top:l[0]}))),f=h+e[j-1].centre[0]-2,f=this.rendEng.createLine([f,b[j-1],f,a.zone.lry-(a.zone.lry-a.zone.uly)/2],{strokeWidth:2,interact:!0}),i.fixed.push(f));this.drawLedgerLines($.map(a.components,function(b){return a.rootStaffPos+b.pitchDiff}),k,2*e[0].centre[0],
c)}this.drawing=this.rendEng.draw(i,{group:!0,selectable:a.props.interact,eleRef:a})[0];$(a).trigger("mUpdateBoundingBox",this.drawing)};Toe.View.NeumeView.prototype.render=function(a){this.drawNeume(a)};Toe.View.NeumeView.prototype.updateDrawing=function(a){this.drawing&&this.rendEng.canvas.remove(this.drawing);this.ledgerLines&&(this.rendEng.canvas.remove(this.ledgerLines),this.ledgerLines=null);this.drawNeume(a);this.selectDrawing();this.rendEng.repaint()};
