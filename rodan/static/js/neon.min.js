...
$.each(g.objects,function(a,b){f.push(b)});$.each(f,function(a,d){var h=d.eleRef;if(h instanceof Toe.Model.Clef){var l=d.left,i=d.top;1<f.length&&(l=g.left+d.left,i=g.top+d.top);l=h.staff.ohSnap({x:l,y:i},null,{ignoreEle:h});l=-Math.round((l.y-h.staff.zone.uly)/(h.staff.delta_y/2));h.setStaffPosition(l);var k=h.staff.getPitchedElements({neumes:!0,custos:!1});0<k.length&&h.staff.getActingClefByEle(k[0])==h&&(l=b.page.getPreviousStaff(h.staff))&&b.handleUpdatePrevCustos(k[0].components[0].pname,k[0].components[0].oct,
l);var l=$.map(h.staff.getPitchedElements({clef:h}),function(a){if(a instanceof Toe.Model.Neume){var d=[];$.each(a.components,function(a,b){d.push({pname:b.pname,oct:b.oct})});return{id:a.id,noteInfo:d}}if(a instanceof Toe.Model.Custos){var c=b.getOutputBoundingBox([a.zone.ulx,a.zone.uly,a.zone.lrx,a.zone.lry]);$.post(b.prefix+"/edit/"+b.fileName+"/move/custos",{id:a.id,ulx:c[0],uly:c[1],lrx:c[2],lry:c[3],csrfmiddlewaretoken:b.csrf_token}).error(function(){$("#alert > p").text("Server failed to move custos. Client and server are not synchronized.");
$("#alert").animate({opacity:1},100)})}}),i=h.staff.props.numLines+h.props.staffPos/2,k=b.getOutputBoundingBox([h.zone.ulx,h.zone.uly,h.zone.lrx,h.zone.lry]),n={id:h.id,line:i,ulx:k[0],uly:k[1],lrx:k[2],lry:k[3],pitchInfo:l};$.post(b.prefix+"/edit/"+b.fileName+"/move/clef",{data:JSON.stringify(n),csrfmiddlewaretoken:b.csrf_token}).error(function(){$("#alert > p").text("Server failed to move clef. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)})}else if(h instanceof
Toe.Model.Neume){l=d.left;i=d.top;1<f.length&&(l=g.left+d.left,i=g.top+d.top);var o={x:l,y:h.staff.zone.uly-h.rootStaffPos*h.staff.delta_y/2-c},p=b.page.getClosestStaff(o),q=p.ohSnap(o,d.currentWidth,{ignoreEle:h}),l=Math.round((p.zone.uly-q.y)/(p.delta_y/2)),k=q.x-d.currentWidth/2,i=i-d.currentHeight/2-(o.y-q.y),k=[k,i,k+d.currentWidth,i+d.currentHeight];h.setBoundingBox(k);o=h.rootStaffPos;$.each(h.components,function(a,b){var c=p.calcPitchFromCoords({x:q.x,y:q.y-p.delta_y/2*b.pitchDiff});b.setPitchInfo(c.pname,
c.oct)});$(h).trigger("vEraseDrawing");h.staff.removeElementByRef(h);i=p.addNeume(h);1==f.length&&$(h).trigger("vSelectDrawing");k=b.getOutputBoundingBox([h.zone.ulx,h.zone.uly,h.zone.lrx,h.zone.lry]);n={id:h.id,ulx:k[0],uly:k[1],lrx:k[2],lry:k[3]};o!=l?(n.pitchInfo=[],$.each(h.components,function(a,b){n.pitchInfo.push({pname:b.pname,oct:b.oct})}),h==p.elements[1]&&(l=b.page.getPreviousStaff(p))&&b.handleUpdatePrevCustos(h.components[0].pname,h.components[0].oct,l)):n.pitchInfo=null;i+1<p.elements.length?
n.beforeid=p.elements[i+1].id:(l=b.page.getNextStaff(p),n.beforeid=l.id);$.post(b.prefix+"/edit/"+b.fileName+"/move/neume",{data:JSON.stringify(n),csrfmiddlewaretoken:b.csrf_token}).error(function(){$("#alert > p").text("Server failed to move neume. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)})}else if(h instanceof Toe.Model.Division){l=d.left;i=d.top;1<f.length&&(l+=g.left,i+=g.top);o={x:l,y:i};l=b.page.getClosestStaff(o);q=l.ohSnap(o,d.currentWidth,{x:!0,y:!1});
switch(h.type){case Toe.Model.Division.Type.div_small:q.y=l.zone.uly;break;case Toe.Model.Division.Type.div_minor:q.y=l.zone.uly+(l.zone.lry-l.zone.uly)/2;break;case Toe.Model.Division.Type.div_major:q.y=l.zone.uly+(l.zone.lry-l.zone.uly)/2;break;case Toe.Model.Division.Type.div_final:q.y=l.zone.uly+(l.zone.lry-l.zone.uly)/2}h.staff.removeElementByRef(h);b.rendEng.canvas.remove(d);b.rendEng.repaint();k=q.x-d.currentWidth/2;i=q.y-d.currentHeight/2;k=[k,i,k+d.currentWidth,i+d.currentHeight];h.setBoundingBox(k);
k=l.addDivision(h);1==f.length&&h.selectDrawing();i=null;k+1<l.elements.length?i=l.elements[k+1].id:(l=b.page.getNextStaff(p),i=l.id);k=b.getOutputBoundingBox([h.zone.ulx,h.zone.uly,h.zone.lrx,h.zone.lry]);$.post(b.prefix+"/edit/"+b.fileName+"/move/division",{id:h.id,ulx:k[0],uly:k[1],lrx:k[2],lry:k[3],beforeid:i,csrfmiddlewaretoken:b.csrf_token}).error(function(){$("#alert > p").text("Server failed to move division. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)})}});
...
f.zone.lry]),c=f.typeid,i=$.map(f.components,function(a){return a.props.type}),a=JSON.stringify({nids:m.join(","),typeid:c,headShapes:i,ulx:a[0],uly:a[1],lrx:a[2],lry:a[3]});$.post(b.prefix+"/edit/"+b.fileName+"/neumify",{data:a,csrfmiddlewaretoken:b.csrf_token},function(a){f.id=a.id}).error(function(){$("#alert > p").text("Server failed to neumify selected neumes. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)});b.rendEng.canvas.discardActiveGroup();$(f).trigger("vSelectDrawing");
...
g),e=[];a(d).each(function(b,c){a(c).is("sb")&&e.push(b)});var h=a("neume, sb",g),i=[];a(h).each(function(b,c){a(c).is("sb")&&i.push(b)});var k=a("division, sb",g),l=[];a(k).each(function(b,c){a(c).is("sb")&&l.push(b)});var m=a("custos, sb",g),u=[];a(m).each(function(b,c){a(c).is("sb")&&u.push(b)});a("sb",g).each(function(r,v){j.debug&&console.log("system "+(r+1));var s=a(v).attr("systemref"),s=a(a(g).find("system[xml\\:id="+s+"]")[0]).attr("facs"),s=a(g).find("zone[xml\\:id="+s+"]")[0],s=c(s);j.debug&&
f.outlineBoundingBox(s,{fill:"blue"});var t=new Toe.Model.Staff(s);t.setID(a(v).attr("xml:id"));0==r&&f.calcScaleFromStaff(t,{overwrite:!0});s=new Toe.View.StaffView(f);new Toe.Ctrl.StaffController(t,s);b.addStaff(t);a(d).slice(e[r]+1,e[r+1]).each(function(b,d){var e=a(d).attr("shape"),h=parseInt(a(d).attr("line")),h=2*-(t.props.numLines-h),i=a(d).attr("facs"),i=a(g).find("zone[xml\\:id="+i+"]")[0],i=c(i);j.debug&&f.outlineBoundingBox(i,{fill:"red"});e=new Toe.Model.Clef(e,{staffPos:h});e.setID(a(d).attr("xml:id"));
e.setBoundingBox(i);h=new Toe.View.ClefView(f);new Toe.Ctrl.ClefController(e,h);t.addClef(e);j.debug&&console.log("clef: "+e.name)});a(h).slice(i[r]+1,i[r+1]).each(function(b,d){var e=new Toe.Model.Neume,h=a(g).find("zone[xml\\:id="+a(d).attr("facs")+"]")[0],h=c(h);j.debug&&f.outlineBoundingBox(h,{fill:"green"});e.neumeFromMei(d,h,t);h=new Toe.View.NeumeView(f);new Toe.Ctrl.NeumeController(e,h);t.addNeume(e);j.debug&&console.log("neume: "+e.name)});a(k).slice(l[r]+1,l[r+1]).each(function(b,d){var e=
a(g).find("zone[xml\\:id="+a(d).attr("facs")+"]")[0],h=c(e);j.debug&&f.outlineBoundingBox(h,{fill:"yellow"});var e="div_"+a(d).attr("form"),i=new Toe.Model.Division(e);i.setBoundingBox(h);i.setID(a(d).attr("xml:id"));h=new Toe.View.DivisionView(f);new Toe.Ctrl.DivisionController(i,h);t.addDivision(i);j.debug&&console.log("division: "+e)});a(m).slice(u[r]+1,u[r+1]).each(function(b,d){var e=a(g).find("zone[xml\\:id="+a(d).attr("facs")+"]")[0],e=c(e);j.debug&&f.outlineBoundingBox(e,{fill:"purple"});
var h=a(d).attr("pname"),i=parseInt(a(d).attr("oct")),h=new Toe.Model.Custos(h,i);h.setBoundingBox(e);h.setID(a(d).attr("xml:id"));e=new Toe.View.CustosView(f);new Toe.Ctrl.CustosController(h,e);t.setCustos(h);j.debug&&console.log("custos")})});f.repaint()},l=function(b){console.log("loading SVG glyphs ...");return a.get(j.glyphpath,function(c){var d={};a(c).find("svg").each(function(b,c){var e=a("<lol>").append(a(c).clone()).remove().html();fabric.loadSVGFromString(e,function(b){gID=a(c).find("path").attr("id");
d[gID]=new Toe.Model.Glyph(gID,b[0])})});b.setGlyphs(d)})},i=function(){var b=a.Deferred();j.autoLoad&&j.meipath?a.get(j.meipath,function(a){console.log("loading MEI file ...");g=a;b.resolve()}):b.resolve();return b.promise()},k=function(){var b=new Toe.Model.Page,c=a("<canvas>").attr("id",j.canvasid),d=[j.width,j.height];j.autoLoad&&(d=j.bgimgpath?[j.orig_width,j.orig_height]:b.calcDimensions(a(g).find("zone")),b.setPageScale(j.width/j.orig_width));b.setDimensions(Math.round(d[0]),Math.round(d[1]));
c.attr("width",b.width);c.attr("height",b.height);c.attr("style","border: 4px black solid;");e.prepend(c);c={renderOnAddition:!1};j.bgimgpath&&a.extend(c,{backgroundImage:j.bgimgpath,backgroundImageOpacity:j.backgroundImageOpacity,backgroundImageStretch:!1});f.setCanvas(new fabric.Canvas(j.canvasid,c));if(j.debug){var i=a("<div>").attr("id","fps");i.attr("style","color: red; font-size: 200%");e.prepend(i);f.canvas.onFpsUpdate=function(b){a(i).html("FPS: "+b)}}c=new Toe.View.PageView(f);new Toe.Ctrl.PageController(b,
c);j.autoLoad&&g&&h(b);new Toe.View.GUI(j.prefix,j.pageid,f,b,j.csrf_token,{sldr_bgImgOpacity:j.bgimgpath,initBgImgOpacity:j.backgroundImageOpacity});console.log("Neon.js ready ("+(new Date-m)+"ms)")};(function(){m=new Date;f=new Toe.View.RenderEngine;a.when(l(f),i()).then(k,function(){console.log("Failure to load the mei file, glyphs, or background image")})})()};a.fn.neon=function(c){return this.each(function(){var d=a(this);if(!d.data("neon")){var e=new b(this,c);d.data("neon",e)}})}})(jQuery);function SearchTree(){this.rootNode=null;this.numNodes=0}function SearchTreeNode(a){this.payload=a;this.children={};this.numChildren=0}
...
e[0].centre[0]);n=e[0].clone().set({left:k[0],top:b[0]});j.modify.push(n);h=this.rendEng.getGlyph("porrect_1");l=h.clone().set({left:k[0]+e[0].centre[0]+h.centre[0]-1,top:b[1]+h.centre[1]/2});j.modify.push(l);var f=l.left-h.centre[0],i=b[0],p=l.top+h.centre[1],i=this.rendEng.createLine([f,b[1],f,i],{strokeWidth:2,interact:!0});j.fixed.push(i);"torculus.resupinus.1"==a.typeid?k.push(l.left+h.centre[0]-e[3].centre[0]):k.push(l.left+h.centre[0]+e[3].centre[0]);f=e[3].clone().set({left:k[1],top:b[3]});
j.modify.push(f);i=l.left+h.centre[0]-1;i=this.rendEng.createLine([i,b[3],i,b[2]],{strokeWidth:2,interact:!0});j.fixed.push(i);"torculus.resupinus.2"==a.typeid&&(i=f.left+e[3].centre[0]-1,i=this.rendEng.createLine([i,b[4],i,b[3]],{strokeWidth:2,interact:!0}),j.fixed.push(i));for(f=4;f<a.components.length;f++)k.push(k[k.length-1]+2*e[f].centre[0]-1),i=e[f].clone().set({left:k[k.length-1],top:b[f]}),j.modify.push(i);$.each(a.components,function(a,f){if(f.hasOrnament("dot")){var g=d.bestDotPlacements(c,
b,a);g.length>0&&j.modify.push(m.clone().set({left:k[a]+2*e[1].centre[0],top:g[0]}))}});this.drawLedgerLines($.map(a.components,function(b){return a.rootStaffPos+b.pitchDiff}),k,2*e[0].centre[0],c);break;case "podatus":k=[];k.push(a.zone.ulx+e[0].centre[0]);n=e[0].clone().set({left:k[0],top:b[0]});j.modify.push(n);i=n.left+e[0].centre[0];i=this.rendEng.createLine([i,b[0],i,b[1]],{strokeWidth:2,interact:!0});j.fixed.push(i);k.push(k[0]+2*e[1].centre[0]);o=e[1].clone().set({left:k[1],top:b[1]});j.modify.push(o);
$.each(a.components,function(a,f){if(f.hasOrnament("dot")){var g=d.bestDotPlacements(c,b,a);0<g.length&&j.modify.push(m.clone().set({left:n.left+2*e[1].centre[0],top:g[0]}))}});this.drawLedgerLines($.map(a.components,function(b){return a.rootStaffPos+b.pitchDiff}),k,2*e[0].centre[0],c);break;case "epiphonus":k=[];h=this.rendEng.getGlyph("podatus_ep");k.push(a.zone.ulx+h.centre[0]);n=h.clone().set({left:k[0],top:b[0]});j.modify.push(n);i=n.left+h.centre[0]-2;i=this.rendEng.createLine([i,b[0],i,b[1]],
{strokeWidth:2,interact:!0});j.fixed.push(i);f=this.rendEng.getGlyph("liques_down");k.push(k[0]+h.centre[0]-f.centre[0]);o=f.clone().set({left:k[1],top:b[1]+f.centre[1]/2});j.modify.push(o);$.each(a.components,function(a,f){if(f.hasOrnament("dot")){var g=d.bestDotPlacements(c,b,a);0<g.length&&j.modify.push(m.clone().set({left:n.left+2*e[1].centre[0],top:g[0]}))}});this.drawLedgerLines($.map(a.components,function(b){return a.rootStaffPos+b.pitchDiff}),k,2*e[0].centre[0],c);break;case "porrectus":h=
this.rendEng.getGlyph("porrect_1");l=h.clone().set({left:a.zone.ulx+h.centre[0],top:b[0]+h.centre[1]/2});j.modify.push(l);f=l.left-h.centre[0]+1;i=a.zone.lry;p=l.top+h.centre[1];a.zone.lry<l.top+p&&(i=p);i=this.rendEng.createLine([f,b[0],f,i],{strokeWidth:2,interact:!0});j.fixed.push(i);k=l.left+h.centre[0]-e[2].centre[0];f=e[2].clone().set({left:k,top:b[2]});i=f.left+e[2].centre[0]-1;i=this.rendEng.createLine([i,b[2],i,b[1]],{strokeWidth:2,interact:!0});j.fixed.push(i);j.modify.push(f);a.components[2].hasOrnament("dot")&&
(l=this.bestDotPlacements(c,b,2),0<l.length&&j.modify.push(m.clone().set({left:f.left+2*e[2].centre[0],top:l[0]})));this.drawLedgerLines([a.rootStaffPos+a.components[2].pitchDiff],[k],2*e[2].centre[0],c);break;case "porrectus.flexus":h=this.rendEng.getGlyph("porrect_1");l=h.clone().set({left:a.zone.ulx+h.centre[0],top:b[0]+h.centre[1]/2});j.modify.push(l);f=l.left-h.centre[0]+1;i=a.zone.lry;p=l.top+h.centre[1];a.zone.lry<l.top+p&&(i=p);i=this.rendEng.createLine([f,b[0],f,i],{strokeWidth:2,interact:!0});
j.fixed.push(i);k=[];k.push(l.left+h.centre[0]+e[2].centre[0]);f=e[2].clone().set({left:k[0],top:b[2]});i=k[0]-e[2].centre[0]-1;i=this.rendEng.createLine([i,b[2],i,b[1]],{strokeWidth:2,interact:!0});j.fixed.push(i);j.modify.push(f);for(f=3;f<a.components.length;f++)k.push(k[k.length-1]+2*e[f].centre[0]-1),i=e[f].clone().set({left:k[k.length-1],top:b[f]}),j.modify.push(i);for(f=2;f<a.components.length;f++)a.components[f].hasOrnament("dot")&&(l=d.bestDotPlacements(c,b,it),0<l.length&&j.modify.push(m.clone().set({left:n.left+
2*e[1].centre[0],top:l[0]})));break;case "scandicus.1":case "scandicus.2":case "scandicus.3":case "scandicus.4":i=a.components.length;p=this.rendEng.getGlyph("pes");h=a.zone.ulx-e[0].centre[0];k=[];for(f=0;f<i-1;f+=2){yoffset=0;1==Math.abs(a.components[f+1].pitchDiff-a.components[f].pitchDiff)&&(yoffset=1);h+=2*p.centre[0]-1;k.push(h);l=p.clone().set({left:h,top:b[f]-p.centre[1]/2+yoffset});j.modify.push(l);l=h+p.centre[0]-1;l=this.rendEng.createLine([l,b[f],l,b[f+1]],{strokeWidth:2,interact:!0});
j.fixed.push(l);k.push(h);o=e[f+1].clone().set({left:h,top:b[f+1]-yoffset});j.modify.push(o);for(g=f;g<f+2;g++)a.components[g].hasOrnament("dot")&&(l=this.bestDotPlacements(c,b,g),0<l.length&&j.modify.push(m.clone().set({left:o.left+2*e[g].centre[0],top:l[0]})))}1==a.components.length%2&&(h+=2*e[i-1].centre[0]-1,k.push(h),f=e[i-1].clone().set({left:h,top:b[i-1]}),j.modify.push(f),a.components[i-1].hasOrnament("dot")&&(l=this.bestDotPlacements(c,b,i-1),0<l.length&&j.modify.push(m.clone().set({left:f.left+
2*e[i-1].centre[0],top:l[0]}))),f=h+e[i-1].centre[0]-2,f=this.rendEng.createLine([f,b[i-1],f,a.zone.lry-(a.zone.lry-a.zone.uly)/2],{strokeWidth:2,interact:!0}),j.fixed.push(f));this.drawLedgerLines($.map(a.components,function(b){return a.rootStaffPos+b.pitchDiff}),k,2*e[0].centre[0],c);break;case "scandicus.flexus.1":k=[];k.push(a.zone.ulx+e[0].centre[0]);n=e[0].clone().set({left:k[0],top:b[0]});j.modify.push(n);i=n.left+e[0].centre[0];i=this.rendEng.createLine([i,b[0],i,b[1]],{strokeWidth:2,interact:!0});
j.fixed.push(i);k.push(k[0]+2*e[1].centre[0]);o=e[1].clone().set({left:k[1],top:b[1]});j.modify.push(o);for(g=0;2>g;g++)a.components[g].hasOrnament("dot")&&(l=this.bestDotPlacements(c,b,g),0<l.length&&j.modify.push(m.clone().set({left:k[g]+2*e[g].centre[0],top:l[0]})));k.push(k[1]+2*e[2].centre[0]);f=e[2].clone().set({left:k[2],top:b[2]});j.modify.push(f);i=k[2]+e[2].centre[0]-1;i=this.rendEng.createLine([i,b[2],i,b[3]],{strokeWidth:2,interact:!0});j.fixed.push(i);k.push(k[2]+2*e[3].centre[0]);f=
e[3].clone().set({left:k[3],top:b[3]});j.modify.push(f);for(g=2;g<a.components.length;g++)a.components[g].hasOrnament("dot")&&(l=d.bestDotPlacements(c,b,it),0<l.length&&j.modify.push(m.clone().set({left:o.left+2*e[1].centre[0],top:l[0]})));this.drawLedgerLines($.map(a.components,function(b){return a.rootStaffPos+b.pitchDiff}),k,2*e[0].centre[0],c)}this.drawing=this.rendEng.draw(j,{group:!0,selectable:a.props.interact,eleRef:a})[0];$(a).trigger("mUpdateBoundingBox",this.drawing)};
Toe.View.NeumeView.prototype.render=function(a){this.drawNeume(a)};Toe.View.NeumeView.prototype.updateDrawing=function(a){this.drawing&&this.rendEng.canvas.remove(this.drawing);this.ledgerLines&&(this.rendEng.canvas.remove(this.ledgerLines),this.ledgerLines=null);this.drawNeume(a);this.selectDrawing();this.rendEng.repaint()};
