...
return!1}));$("#btn_edit").bind("click.edit",{gui:c,parentDivId:a},this.handleEdit);$("#btn_insert").bind("click.insert",{gui:c,parentDivId:a},this.handleInsert)};
...
a.pname.toUpperCase()+a.oct),$("#info").animate({opacity:1},100)),$("#menu_editpunctum").remove(),$("#menu_editclef").remove())});b.rendEng.canvas.observe("selection:cleared",function(){$("#info").animate({opacity:0},100);$("#menu_editpunctum").remove();$("#menu_editclef").remove()});b.rendEng.canvas.observe("mouse:up",function(a){var a=b.rendEng.canvas.getPointer(a.memo.e),d=b.downCoords.x-a.x,e=b.downCoords.y-a.y;if(b.objMoving){var g=b.rendEng.canvas.getActiveObject();if(g){var f=[];g.eleRef?f.push(g):
$.each(g.objects,function(a,b){f.push(b)});$.each(f,function(a,c){var h=c.eleRef;if(h instanceof Toe.Model.Clef){var i=c.left,k=c.top;1<f.length&&(i=g.left+c.left,k=g.top+c.top);i=h.staff.ohSnap({x:i,y:k},null,{ignoreEle:h});i=-Math.round((i.y-h.staff.zone.uly)/(h.staff.delta_y/2));h.setStaffPosition(i);var m=h.staff.getPitchedElements({neumes:!0,custos:!1});0<m.length&&h.staff.getActingClefByEle(m[0])==h&&(i=b.page.getPreviousStaff(h.staff))&&b.handleUpdatePrevCustos(m[0].components[0].pname,m[0].components[0].oct,
i);var i=$.map(h.staff.getPitchedElements({clef:h}),function(a){if(a instanceof Toe.Model.Neume){var c=[];$.each(a.components,function(a,b){c.push({pname:b.pname,oct:b.oct})});return{id:a.id,noteInfo:c}}if(a instanceof Toe.Model.Custos){var e=b.getOutputBoundingBox([a.zone.ulx,a.zone.uly,a.zone.lrx,a.zone.lry]);$.post(b.apiprefix+"/move/custos",{id:a.id,ulx:e[0],uly:e[1],lrx:e[2],lry:e[3]}).error(function(){$("#alert > p").text("Server failed to move custos. Client and server are not synchronized.");
$("#alert").animate({opacity:1},100)})}}),k=h.staff.props.numLines+h.props.staffPos/2,m=b.getOutputBoundingBox([h.zone.ulx,h.zone.uly,h.zone.lrx,h.zone.lry]),n={id:h.id,line:k,ulx:m[0],uly:m[1],lrx:m[2],lry:m[3],pitchInfo:i};$.post(b.apiprefix+"/move/clef",{data:JSON.stringify(n)}).error(function(){$("#alert > p").text("Server failed to move clef. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)})}else if(h instanceof Toe.Model.Neume){i=c.left;k=c.top;1<f.length&&(i=
g.left+c.left,k=g.top+c.top);var o={x:i,y:h.staff.zone.uly-h.rootStaffPos*h.staff.delta_y/2-e},p=b.page.getClosestStaff(o),q=p.ohSnap(o,c.currentWidth,{ignoreEle:h}),i=Math.round((p.zone.uly-q.y)/(p.delta_y/2)),m=q.x-c.currentWidth/2,k=k-c.currentHeight/2-(o.y-q.y),m=[m,k,m+c.currentWidth,k+c.currentHeight];h.setBoundingBox(m);o=h.rootStaffPos;$.each(h.components,function(a,b){var c=p.calcPitchFromCoords({x:q.x,y:q.y-p.delta_y/2*b.pitchDiff});b.setPitchInfo(c.pname,c.oct)});$(h).trigger("vEraseDrawing");
h.staff.removeElementByRef(h);k=p.addNeume(h);1==f.length&&$(h).trigger("vSelectDrawing");m=b.getOutputBoundingBox([h.zone.ulx,h.zone.uly,h.zone.lrx,h.zone.lry]);n={id:h.id,ulx:m[0],uly:m[1],lrx:m[2],lry:m[3]};o!=i?(n.pitchInfo=[],$.each(h.components,function(a,b){n.pitchInfo.push({pname:b.pname,oct:b.oct})}),h==p.elements[1]&&(i=b.page.getPreviousStaff(p))&&b.handleUpdatePrevCustos(h.components[0].pname,h.components[0].oct,i)):n.pitchInfo=null;k+1<p.elements.length?n.beforeid=p.elements[k+1].id:
(i=b.page.getNextStaff(p),n.beforeid=i.id);$.post(b.apiprefix+"/move/neume",{data:JSON.stringify(n)}).error(function(){$("#alert > p").text("Server failed to move neume. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)})}else if(h instanceof Toe.Model.Division){i=c.left;k=c.top;1<f.length&&(i+=g.left,k+=g.top);o={x:i,y:k};i=b.page.getClosestStaff(o);q=i.ohSnap(o,c.currentWidth,{x:!0,y:!1});switch(h.type){case Toe.Model.Division.Type.div_small:q.y=i.zone.uly;break;case Toe.Model.Division.Type.div_minor:q.y=
i.zone.uly+(i.zone.lry-i.zone.uly)/2;break;case Toe.Model.Division.Type.div_major:q.y=i.zone.uly+(i.zone.lry-i.zone.uly)/2;break;case Toe.Model.Division.Type.div_final:q.y=i.zone.uly+(i.zone.lry-i.zone.uly)/2}h.staff.removeElementByRef(h);b.rendEng.canvas.remove(c);b.rendEng.repaint();m=q.x-c.currentWidth/2;k=q.y-c.currentHeight/2;m=[m,k,m+c.currentWidth,k+c.currentHeight];h.setBoundingBox(m);m=i.addDivision(h);1==f.length&&h.selectDrawing();k=null;m+1<i.elements.length?k=i.elements[m+1].id:(i=b.page.getNextStaff(p),
k=i.id);m=b.getOutputBoundingBox([h.zone.ulx,h.zone.uly,h.zone.lrx,h.zone.lry]);$.post(b.apiprefix+"/move/division",{id:h.id,ulx:m[0],uly:m[1],lrx:m[2],lry:m[3],beforeid:k}).error(function(){$("#alert > p").text("Server failed to move division. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)})}else h instanceof Toe.Model.Custos&&(i=c.left,k=c.top,1<f.length&&(c.left=i+d,c.top=k+e))});1<f.length&&b.rendEng.canvas.discardActiveGroup();b.rendEng.repaint()}b.objMoving=!1}});
$("#btn_delete").unbind("click");$("#btn_neumify").unbind("click");$("#btn_ungroup").unbind("click");$("#btn_delete").bind("click.edit",{gui:b},b.handleDelete);$("#btn_neumify").bind("click.edit",{gui:b},b.handleNeumify);$("#btn_neumify_liquescence").bind("click.edit",{gui:b,modifier:"alt"},b.handleNeumify);$("#btn_ungroup").bind("click.edit",{gui:b},b.handleUngroup);return!1};
...
$("#alert").animate({opacity:1},100)}):$.post(b.apiprefix+"/insert/dot",c).error(function(){$("#alert > p").text("Server failed to add a dot to the punctum. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)});$(this).toggleClass("active");return!1};
...
shape:a,ulx:e[0],uly:e[1],lrx:e[2],lry:e[3],pitchInfo:d})}).error(function(){$("#alert > p").text("Server failed to update clef shape. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)});$(this).toggleClass("active")}return!1};
Toe.View.GUI.prototype.handleDelete=function(a){var b=a.data.gui;toDelete={clefs:[],nids:[],dids:[],cids:[]};var c=function(a){var c=a.eleRef,e=c.staff,d=e.getPreviousClef(c),g=e.getPitchedElements(c);e.removeElementByRef(c);e.updatePitchedElements(d);e=$.map(g,function(a){if(a instanceof Toe.Model.Neume){var c=[];$.each(a.components,function(a,b){c.push({pname:b.pname,oct:b.oct})});return{id:a.id,noteInfo:c}}if(a instanceof Toe.Model.Custos){var e=b.getOutputBoundingBox([a.zone.ulx,a.zone.uly,a.zone.lrx,
a.zone.lry]);$.post(b.apiprefix+"/move/custos",{id:a.id,ulx:e[0],uly:e[1],lrx:e[2],lry:e[3]}).error(function(){$("#alert > p").text("Server failed to move custos. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)})}});toDelete.clefs.push({id:c.id,pitchInfo:e});b.rendEng.canvas.remove(a);b.rendEng.canvas.discardActiveObject()},d=function(a){var a=a.eleRef,c=a.staff.getPitchedElements({neumes:!0,custos:!1});a.staff.removeElementByRef(a);toDelete.nids.push(a.id);b.rendEng.canvas.discardActiveObject();
if(1==c.length){var e=b.page.getPreviousStaff(a.staff);e&&e.custos&&(e.custos.eraseDrawing(),e.removeElementByRef(e.custos),$.post(b.apiprefix+"/delete/custos",{ids:e.custos.id}).error(function(){$("#alert > p").text("Server failed to delete custos. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)}),e.custos=null)}else if(a==c[0]&&(e=b.page.getPreviousStaff(a.staff))&&e.custos){var a=e.custos,d=c[1],c=d.components[0].pname,d=d.components[0].oct,g=e.getActingClefByEle(a),
e=e.calcStaffPosFromPitch(c,d,g);a.pname=c;a.oct=d;a.setRootStaffPos(e);e=b.getOutputBoundingBox([a.zone.ulx,a.zone.uly,a.zone.lrx,a.zone.lry]);$.post(b.apiprefix+"/move/custos",{id:a.id,pname:c,oct:d,ulx:e[0],uly:e[1],lrx:e[2],lry:e[3]}).error(function(){$("#alert > p").text("Server failed to move custos. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)})}},e=function(a){var c=a.eleRef;c.staff.removeElementByRef(c);toDelete.dids.push(c.id);b.rendEng.canvas.remove(a);
b.rendEng.canvas.discardActiveObject()},g=function(a){var c=a.eleRef;c.staff.removeElementByRef(c);toDelete.cids.push(c.id);b.rendEng.canvas.remove(a);b.rendEng.canvas.discardActiveObject()};if(a=b.rendEng.canvas.getActiveObject())a.eleRef instanceof Toe.Model.Clef&&a.eleRef.staff.elements[0]!=a.eleRef?c(a):a.eleRef instanceof Toe.Model.Neume?d(a):a.eleRef instanceof Toe.Model.Division?e(a):a.eleRef instanceof Toe.Model.Custos&&g(a),b.rendEng.repaint();else if(a=b.rendEng.canvas.getActiveGroup())$.each(a.getObjects(),
function(a,b){b.eleRef instanceof Toe.Model.Clef&&b.eleRef.staff.elements[0]!=b.eleRef?c(b):b.eleRef instanceof Toe.Model.Neume?d(b):b.eleRef instanceof Toe.Model.Division?e(b):b.eleRef instanceof Toe.Model.Custos&&g(b)}),b.rendEng.canvas.discardActiveGroup(),b.rendEng.repaint();0<toDelete.clefs.length&&$.post(b.apiprefix+"/delete/clef",{data:JSON.stringify(toDelete.clefs)}).error(function(){$("#alert > p").text("Server failed to delete clef. Client and server are not synchronized.");$("#alert").animate({opacity:1},
...
$.post(b.apiprefix+"/delete/custos",{ids:toDelete.cids.join(",")}).error(function(){$("#alert > p").text("Server failed to delete custos. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)});return!1};
Toe.View.GUI.prototype.handleNeumify=function(a){var b=a.data.gui,a=a.data.modifier,c=b.rendEng.canvas.getActiveGroup();if(c){var d=[],e=null;$.each(c.getObjects(),function(a,b){b.eleRef instanceof Toe.Model.Neume&&(e||(e=b.eleRef.staff),b.eleRef.staff==e&&d.push(b))});if(2>d.length)return;d.sort(function(a,b){return a.eleRef.zone.ulx-b.eleRef.zone.ulx});var g=new Toe.Model.Neume({modifier:a});numPunct=0;var f=[],l=Number.MAX_VALUE,j=Number.MAX_VALUE,h=Number.MIN_VALUE;$.each(d,function(a,d){$.merge(g.components,
d.eleRef.components);numPunct+=d.eleRef.components.length;f.push(d.eleRef.id);var i=c.top+d.top;l=Math.min(l,c.left+d.left-d.currentHeight/2);j=Math.min(j,i-d.currentHeight/2);h=Math.max(h,i+d.currentHeight/2);e.removeElementByRef(d.eleRef);b.rendEng.canvas.remove(d)});g.setBoundingBox([l,j,l+numPunct*b.punctWidth,h]);a=new Toe.View.NeumeView(b.rendEng,b.dwgLib);new Toe.Ctrl.NeumeController(g,a);e.addNeume(g);var a=b.getOutputBoundingBox([g.zone.ulx,g.zone.uly,g.zone.lrx,g.zone.lry]),i=g.typeid,k=
$.map(g.components,function(a){return a.props.type}),a=JSON.stringify({nids:f.join(","),typeid:i,headShapes:k,ulx:a[0],uly:a[1],lrx:a[2],lry:a[3]});$.post(b.apiprefix+"/neumify",{data:a},function(a){g.id=JSON.parse(a).id}).error(function(){$("#alert > p").text("Server failed to neumify selected neumes. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)});b.rendEng.canvas.discardActiveGroup();$(g).trigger("vSelectDrawing");b.rendEng.repaint()}return!1};
Toe.View.GUI.prototype.handleUngroup=function(a){var b=a.data.gui,c=[];(a=b.rendEng.canvas.getActiveObject())?a.eleRef instanceof Toe.Model.Neume&&1<a.eleRef.components.length&&c.push(a):(a=b.rendEng.canvas.getActiveGroup())&&$.each(a.getObjects(),function(a,b){b.eleRef instanceof Toe.Model.Neume&&1<b.eleRef.components.length&&c.push(b)});var d=[],e=[],g=[];$.each(c,function(a,c){d.push(c.eleRef.id);var j=[],h=c.eleRef.zone.ulx;c.eleRef.staff.removeElementByRef(c.eleRef);b.rendEng.canvas.remove(c);
$.each(c.eleRef.components,function(a,e){var d=new Toe.Model.Neume;d.components.push(e);var f=c.eleRef.staff.zone.uly-(c.eleRef.rootStaffPos+e.pitchDiff)*c.eleRef.staff.delta_y/2-b.punctHeight/2;d.setBoundingBox([h+a*b.punctWidth,f,h+(a+1)*b.punctWidth,f+b.punctHeight]);f=new Toe.View.NeumeView(b.rendEng,b.dwgLib);new Toe.Ctrl.NeumeController(d,f);c.eleRef.staff.addNeume(d);f=b.getOutputBoundingBox([d.zone.ulx,d.zone.uly,d.zone.lrx,d.zone.lry]);j.push({ulx:f[0],uly:f[1],lrx:f[2],lry:f[3]});g.push(d)});
e.push(j)});a=JSON.stringify({nids:d.join(","),bbs:e});$.post(b.apiprefix+"/ungroup",{data:a},function(a){var b=JSON.parse(a).nids,b=$.map(b,function(a){return a});$.each(g,function(a,c){c.id=b[a]})}).error(function(){$("#alert > p").text("Server failed to ungroup selected neumes. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)});b.rendEng.canvas.discardActiveObject();b.rendEng.canvas.discardActiveGroup();b.rendEng.repaint();return!1};
...
{gui:b},b.handleInsertPunctum);$("#rad_division").bind("click.insert",{gui:b},b.handleInsertDivision);$("#rad_clef").bind("click.insert",{gui:b},b.handleInsertClef);$("#rad_punctum").trigger("click");return!1};
...
l=a.pname,j=a.oct,a={pname:l,oct:j},h=[];c&&(h.push(new Toe.Model.Ornament("dot",{form:"aug"})),a.dotform="aug");f.addComponent("punctum",l,j,{ornaments:h});h=new Toe.View.NeumeView(b.rendEng,b.dwgLib);new Toe.Ctrl.NeumeController(f,h);h=d.addNeume(f);if(1==h){var i=b.page.getPreviousStaff(d);i&&b.handleUpdatePrevCustos(l,j,i)}l=b.getOutputBoundingBox([f.zone.ulx,f.zone.uly,f.zone.lrx,f.zone.lry]);a.ulx=l[0];a.uly=l[1];a.lrx=l[2];a.lry=l[3];if(h+1<d.elements.length)a.beforeid=d.elements[h+1].id;else if(d=
b.page.getNextStaff(d))a.beforeid=d.id;$.post(b.apiprefix+"/insert/neume",a,function(a){f.id=JSON.parse(a).id}).error(function(){$("#alert > p").text("Server failed to insert neume. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)})});$("#chk_dot").bind("click.insert",function(){c=c?!1:!0;d(!1)});return!1};
...
new Toe.Ctrl.DivisionController(g,f);f=d.addDivision(g);a=b.getOutputBoundingBox(a);a={type:g.key.slice(4),ulx:a[0],uly:a[1],lrx:a[2],lry:a[3]};f+1<d.elements.length?a.beforeid=d.elements[f+1].id:(f=b.page.getNextStaff(d),a.beforeid=f.id);$.post(b.apiprefix+"/insert/division",a,function(a){g.id=JSON.parse(a).id}).error(function(){$("#alert > p").text("Server failed to insert division. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)})});$("#rad_small").bind("click.insert",
function(){b.divisionDwg&&(b.rendEng.canvas.remove(b.divisionDwg),b.divisionDwg=null);c="div_small";return!1});$("#rad_minor").bind("click.insert",function(){b.divisionDwg&&(b.rendEng.canvas.remove(b.divisionDwg),b.divisionDwg=null);c="div_minor";return!1});$("#rad_major").bind("click.insert",function(){b.divisionDwg&&(b.rendEng.canvas.remove(b.divisionDwg),b.divisionDwg=null);c="div_major";return!1});$("#rad_final").bind("click.insert",function(){b.divisionDwg&&(b.rendEng.canvas.remove(b.divisionDwg),
b.divisionDwg=null);c="div_final";return!1});$("#rad_small").trigger("click");return!1};
...
a.zone.lry]);$.post(b.apiprefix+"/move/custos",{id:a.id,ulx:d[0],uly:d[1],lrx:d[2],lry:d[3]}).error(function(){$("#alert > p").text("Server failed to move custos. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)})}});$.post(b.apiprefix+"/insert/clef",{data:JSON.stringify(a)},function(a){f.id=JSON.parse(a).id}).error(function(){$("#alert > p").text("Server failed to insert clef. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)})});$("#rad_doh").unbind("click");
$("#rad_fah").unbind("click");$("#rad_doh").bind("click.insert",function(){if(!$(this).hasClass("active")){var a={left:-50,top:-50};b.clefDwg&&(b.rendEng.canvas.remove(b.clefDwg),a={left:b.clefDwg.left,top:b.clefDwg.top});a=b.rendEng.getGlyph("c_clef").clone().set($.extend(a,{opacity:0.6}));b.clefDwg=b.rendEng.draw({fixed:[],modify:[a]},{opacity:0.6,selectable:!1,repaint:!0})[0];c="c"}return!1});$("#rad_fah").bind("click.insert",function(){if(!$(this).hasClass("active")){var a={left:-50,top:-50};
b.clefDwg&&(b.rendEng.canvas.remove(b.clefDwg),a={left:b.clefDwg.left,top:b.clefDwg.top});a=b.rendEng.getGlyph("f_clef").clone().set($.extend(a,{opacity:0.6}));b.clefDwg=b.rendEng.draw({fixed:[],modify:[a]},{opacity:0.6,selectable:!1,repaint:!0})[0];c="f"}return!1});$("#rad_doh").trigger("click");return!1};
Toe.View.GUI.prototype.handleUpdatePrevCustos=function(a,b,c){var d=c.custos;if(d){d.setRootNote(a,b);var e=c.getActingClefByEle(d);d.setRootStaffPos(c.calcStaffPosFromPitch(a,b,e));e=this.getOutputBoundingBox([d.zone.ulx,d.zone.uly,d.zone.lrx,d.zone.lry]);$.post(this.apiprefix+"/move/custos",{id:d.id,pname:a,oct:b,ulx:e[0],uly:e[1],lrx:e[2],lry:e[3]}).error(function(){$("#alert > p").text("Server failed to move custos. Client and server are not synchronized.");$("#alert").animate({opacity:1},100)})}else{var g=
new Toe.Model.Custos(a,b),d=c.zone.lrx-gui.punctWidth/2,e=c.zone.uly;g.setBoundingBox([d,e,d+gui.punctWidth,e+gui.punctHeight]);d=new Toe.View.CustosView(gui.rendEng);new Toe.Ctrl.CustosController(g,d);c.setCustos(g);e=this.getOutputBoundingBox([g.zone.ulx,g.zone.uly,g.zone.lrx,g.zone.lry]);a={id:g.id,pname:a,oct:b,ulx:e[0],uly:e[1],lrx:e[2],lry:e[3]};if(c=gui.page.getNextStaff(c))a.beforeid=c.id;$.post(this.apiprefix+"/insert/custos",a,function(a){g.id=JSON.parse(a).id}).error(function(){$("#alert > p").text("Server failed to insert custos. Client and server are not synchronized.");
...
Toe.Model.NeumeComponent.prototype.hasOrnament=function(a){return $.grep(this.props.ornaments,function(b){return b.key==a.toLowerCase()}).length};Toe.Model.NeumeComponent.prototype.addOrnament=function(a){if(!(a instanceof Toe.Model.Ornament))throw Error("NeumeComponent: Invalid ornament");this.hasOrnament(a.key)||this.props.ornaments.push(a)};Toe.Model.NeumeComponent.prototype.removeOrnament=function(a){this.props.ornaments=$.grep(this.props.ornaments,function(b){return b.key==a.toLowerCase()},!0)};(function(a){var b=function(b,d){var e=a(b),g,f,l,j=a.extend({},{debug:!1,glyphpath:"",meipath:"",bgimgpath:"",bgimgopacity:0.6,apiprefix:"",origwidth:null,origheight:null,dwgLib:"liber",width:1E3},d);a.extend(j,{canvasid:"neon-canvas"});var h=function(b){function c(d){var e=parseInt(a(d).attr("ulx")),f=parseInt(a(d).attr("uly")),g=parseInt(a(d).attr("lrx")),d=parseInt(a(d).attr("lry"));return a.map([e,f,g,d],function(a){return Math.round(b.scale*a)})}var d=a(g).find("surface")[0],e=a("clef, sb",
g),h=[];a(e).each(function(b,c){a(c).is("sb")&&h.push(b)});var i=a("neume, sb",g),k=[];a(i).each(function(b,c){a(c).is("sb")&&k.push(b)});var l=a("division, sb",g),m=[];a(l).each(function(b,c){a(c).is("sb")&&m.push(b)});var n=a("custos, sb",g),u=[];a(n).each(function(b,c){a(c).is("sb")&&u.push(b)});a("sb",g).each(function(r,v){j.debug&&console.log("system "+(r+1));var s=a(v).attr("systemref"),s=a(a(g).find("system[xml\\:id="+s+"]")[0]).attr("facs"),s=a(d).find("zone[xml\\:id="+s+"]")[0],s=c(s);j.debug&&
f.outlineBoundingBox(s,{fill:"blue"});var t=new Toe.Model.Staff(s);t.setID(a(v).attr("xml:id"));0==r&&f.calcScaleFromStaff(t,{overwrite:!0});s=new Toe.View.StaffView(f);new Toe.Ctrl.StaffController(t,s);b.addStaff(t);a(e).slice(h[r]+1,h[r+1]).each(function(b,e){var g=a(e).attr("shape"),h=parseInt(a(e).attr("line")),h=2*-(t.props.numLines-h),i=a(e).attr("facs"),i=a(d).find("zone[xml\\:id="+i+"]")[0],i=c(i);j.debug&&f.outlineBoundingBox(i,{fill:"red"});g=new Toe.Model.Clef(g,{staffPos:h});g.setID(a(e).attr("xml:id"));
g.setBoundingBox(i);h=new Toe.View.ClefView(f);new Toe.Ctrl.ClefController(g,h);t.addClef(g);j.debug&&console.log("clef: "+g.name)});a(i).slice(k[r]+1,k[r+1]).each(function(b,e){var g=new Toe.Model.Neume,h=a(d).find("zone[xml\\:id="+a(e).attr("facs")+"]")[0],h=c(h);j.debug&&f.outlineBoundingBox(h,{fill:"green"});g.neumeFromMei(e,h,t);h=new Toe.View.NeumeView(f,j.dwgLib);new Toe.Ctrl.NeumeController(g,h);t.addNeume(g);j.debug&&console.log("neume: "+g.name)});a(l).slice(m[r]+1,m[r+1]).each(function(b,
e){var g=a(d).find("zone[xml\\:id="+a(e).attr("facs")+"]")[0],h=c(g);j.debug&&f.outlineBoundingBox(h,{fill:"yellow"});var g="div_"+a(e).attr("form"),i=new Toe.Model.Division(g);i.setBoundingBox(h);i.setID(a(e).attr("xml:id"));h=new Toe.View.DivisionView(f);new Toe.Ctrl.DivisionController(i,h);t.addDivision(i);j.debug&&console.log("division: "+g)});a(n).slice(u[r]+1,u[r+1]).each(function(b,e){var g=a(d).find("zone[xml\\:id="+a(e).attr("facs")+"]")[0],g=c(g);j.debug&&f.outlineBoundingBox(g,{fill:"purple"});
...
d[gID]=new Toe.Model.Glyph(gID,b[0])})});b.setGlyphs(d)})},k=function(){console.log("loading background image ...");var b=a.Deferred();j.bgimgpath&&!j.origwidth&&!j.origheight?fabric.Image.fromURL(j.bgimgpath,function(a){j.origwidth=a.width;j.origheight=a.height;b.resolve()}):b.resolve();return b.promise()},m=function(){var b=a.Deferred();j.meipath?a.get(j.meipath,function(a){console.log("loading MEI file ...");g=a;b.resolve()}):b.resolve();return b.promise()},n=function(){var b=new Toe.Model.Page,
c=a("<canvas>").attr("id",j.canvasid),d=[j.origwidth,j.origheight];j.bgimgpath||(d=b.calcDimensions(a(g).find("zone")));b.setPageScale(j.width/d[0]);b.setDimensions(Math.round(d[0]),Math.round(d[1]));c.attr("width",b.width);c.attr("height",b.height);c.attr("style","border: 4px black solid;");e.prepend(c);c={renderOnAddition:!1};j.bgimgpath&&a.extend(c,{backgroundImage:j.bgimgpath,backgroundImageOpacity:j.bgimgopacity,backgroundImageStretch:!0});f.setCanvas(new fabric.Canvas(j.canvasid,c));if(j.debug){var i=
a("<div>").attr("id","fps");i.attr("style","color: red; font-size: 200%");e.prepend(i);f.canvas.onFpsUpdate=function(b){a(i).html("FPS: "+b)}}c=new Toe.View.PageView(f);new Toe.Ctrl.PageController(b,c);g&&h(b);new Toe.View.GUI(j.apiprefix,j.meipath,j.dwgLib,f,b,{sldr_bgImgOpacity:j.bgimgpath,initBgImgOpacity:j.bgimgopacity});console.log("Neon.js ready ("+(new Date-l)+"ms)")};(function(){l=new Date;f=new Toe.View.RenderEngine;a.when(i(f),m(),k()).then(n,function(){console.log("Failure to load the mei file, glyphs, or background image")})})()};
...
Toe.View.RenderEngine.prototype.calcScaleFromStaff=function(a,b){opts={overwrite:!1};$.extend(opts,b);var c=(a.zone.lry-a.zone.uly)/(a.props.numLines-1),c=2*c-0.65*c,d=this.getGlyph("c_clef").clone().height;scale=Math.abs(c/d);opts.overwrite&&this.setGlobalScale(scale);return scale};Toe.View.RenderEngine.prototype.createLine=function(a,b){var c={fill:"rgb(0,0,0)",strokeWidth:3,opacity:1,interact:!1};$.extend(c,b);return new fabric.Line(a,{fill:c.fill,strokeWidth:c.strokeWidth,opacity:c.opacity,selectable:c.interact})};
