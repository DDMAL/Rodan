...
class InteractiveAcquireTestCase(
    ...
    def _acquire(self):
        return self.client.post(
            reverse("interactive-acquire", kwargs={"run_job_uuid": self.test_runjob.pk})
        )
    ...
    def test_not_interactive(self):
        ...
        self.assertEqual(
            response.data["message"], "This RunJob does not accept input now"
        )
    ...
    def test_success_first_working_user(self):
        ...
        self.assertEqual(
            response.data["working_url"],
            "http://testserver"
            + reverse(
                "interactive-working",
                kwargs={
                    "run_job_uuid": self.test_runjob.pk,
                    "working_user_token": str(test_rj.working_user_token),
                    "additional_url": "",
                },
            ),
        )
    ...
    def test_success_continue_working_same_token(self):
        ...
        self.test_runjob.working_user_expiry = timezone.now() + datetime.timedelta(
            seconds=1
        )
        self.test_runjob.save()
        response = self._acquire()
    ...
    def test_fail_others_working(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            response.data["message"], "Previous working user has not expired yet."
        )


class InteractiveWorkingTestCase(
    RodanTestTearDownMixin, APITestCase, RodanTestSetUpMixin
):
    def setUp(self):
        self.setUp_rodan()
        ...
        self.test_resource_in.resource_file.save(
    ...
    def test_not_exist(self):
        response = self.client.get(
            reverse(
                "interactive-working",
                kwargs={
                    "run_job_uuid": uuid.uuid1(),
                    "working_user_token": uuid.uuid1(),
                    "additional_url": "",
                },
            ),
            format="json",
    ...
    def test_not_waiting_for_input(self):
        ...
        response = self.client.get(
            reverse(
                "interactive-working",
                kwargs={
                    "run_job_uuid": self.test_runjob.uuid,
                    "working_user_token": uuid.uuid1(),
                    "additional_url": "",
                },
            )
