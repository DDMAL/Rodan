...
def process(
    ...
    if not all_chars:
        all_chars = perform_ocr.recognize_text_strips(
            image, cc_strips, ocr_model_name, verbose
        )
    all_chars = perform_ocr.handle_abbreviations(all_chars)

    assert (
        len(all_chars) > 0
    ), "Fatal error: No characters were found in the OCR of the given page."
    ...
    tra_align, ocr_align, _ = afw.perform_alignment(
        transcript=list(transcript), ocr=list(ocr), **seq_align_params
    )
    ...
    assert len(all_chars) == len(
        tra_align
    ), "all_chars not same length as alignment: " "{} vs {}".format(
        len(all_chars), len(tra_align)
    )
...
def draw_results_on_page(image, syl_boxes, lines_peak_locs, out_fname):
    ...
    for i, cbox in enumerate(syl_boxes):
        ...
        lr = cbox.lr
        draw.rectangle(
            [ul[0], ul[1] - text_size, ul[0] + text_size * len(cbox.char) * 0.6, ul[1]],
            fill="white",
        )
...
if __name__ == "__main__":
    ...
    text_func = pcc.filename_to_text_func(
        "./csv/123723_Salzinnes.csv", "./csv/mapping.csv"
    )
    ...
    for ind in f_inds:
        ...
        ocr_pickle = None
        text_layer_fname = r"D:\Desktop\rodan resources\aligner\png\{}_text.png".format(
            fname
        )
        ...
        result = process(
            raw_image, transcript, ocr_model, existing_ocr=existing_ocr, verbose=True
        )
        ...
        with open(
            r"D:\Desktop\rodan resources\aligner\out_json\{}.json".format(fname), "w"
        ) as outjson:
        ...
        out_fname = (
            r"D:\Desktop\rodan resources\aligner\out_imgs\aligned_{}.png".format(fname)
        )
