...
export default class ControllerProject extends BaseController {
    ...
    _handleEventProjectShowUsers(options) {
        ...
        var ajaxSettingsAdmins = {
            success: response => this._handleProjectGetAdminsSuccess(response, adminUserCollection),
        ...
        var ajaxSettingsWorkers = {
            success: response => this._handleProjectGetWorkersSuccess(response, workerUserCollection),
            error: response => Radio.channel('rodan').request(RODAN_EVENTS.REQUEST__SYSTEM_HANDLE_ERROR, { response: response }),
            type: 'GET',
            dataType: 'json',
            url: options.project.get('url') + 'workers/'
        };
        Radio.channel('rodan').request(RODAN_EVENTS.REQUEST__SERVER_REQUEST_AJAX, {
            settings: ajaxSettingsAdmins
        });
        Radio.channel('rodan').request(RODAN_EVENTS.REQUEST__SERVER_REQUEST_AJAX, {
            settings: ajaxSettingsWorkers
        });

        // Need collection for all users.
        var collection = new UserCollection();
        collection.fetch();
        var userSelectionView = new BaseViewCollection({
            ...
            childViewOptions: {
                template: _.template($('#template-main_user_selection_item').text()),
                tagName: 'option'
            }
        });
        ...
        var projectAdminsView = new BaseViewCollection({
            ...
            childViewOptions: {
                template: _.template($('#template-main_user_collection_item_remove').text()),
                project: options.project,
                admin: true
            }
        });
        var projectWorkersView = new BaseViewCollection({
            collection: workerUserCollection,
            template: _.template($('#template-main_user_collection').text()),
            childView: ViewUserCollectionItem,
            childViewOptions: {
                template: _.template($('#template-main_user_collection_item_remove').text()),
        ...
        var view = new LayoutViewProjectUsers({
            viewusers: userSelectionView,
    ...
    _handleRequestProjectSave(options) {
        ...
        options.project.save(options.fields, {
            patch: true,
            success: model =>
                Radio.channel('rodan').trigger(RODAN_EVENTS.EVENT__PROJECT_SAVED, {
                    project: model
                })
        });
    }
    ...
    _handleRequestCreateProject(options) {
        ...
        project.save(
            ...
            {
                success: model =>
                    Radio.channel('rodan').trigger(RODAN_EVENTS.EVENT__PROJECT_CREATED, {
                        project: model
                    })
            }
        );
        ...
        if (admins.length > 1) {
            ...
            if (userIndex >= 0) {
                ...
                admins.map(function (value) {
                    usersSendObject[value] = value;
                    return value;
                });
                var ajaxSettings = {
                    success: response =>
                        Radio.channel('rodan').trigger(RODAN_EVENTS.EVENT__PROJECT_REMOVED_USER_ADMIN, {
                            project: options.project
                        }),
                    error: response => Radio.channel('rodan').request(RODAN_EVENTS.REQUEST__SYSTEM_HANDLE_ERROR, { response: response }),
                    type: 'PUT',
                ...
                Radio.channel('rodan').request(RODAN_EVENTS.REQUEST__MODAL_ERROR, {
                    content: 'An error occured trying to remove this User.'
                });
            }
        } else {
            Radio.channel('rodan').request(RODAN_EVENTS.REQUEST__MODAL_ERROR, {
                content: 'At least one project admin, the creator, must exist.'
            });
        }
    }
    ...
    _handleRequestRemoveUserWorker(options) {
        ...
        if (users.length > 0) {
            ...
            if (userIndex >= 0) {
                ...
                var ajaxSettings = {
                    success: response =>
                        Radio.channel('rodan').trigger(RODAN_EVENTS.EVENT__PROJECT_REMOVED_USER_WORKER, {
                            project: options.project
                        }),
                    error: response => Radio.channel('rodan').request(RODAN_EVENTS.REQUEST__SYSTEM_HANDLE_ERROR, { response: response }),
                    type: 'PUT',
    ...
    _handleEventProjectRemovedUser(options) {
        this._activeProject.fetch();
        Radio.channel('rodan').trigger(RODAN_EVENTS.EVENT__PROJECT_USERS_SELECTED, {
