...
def simple_binarise_image(page,threshold_value):
        path_to_img = page.path_to_image.encode('ascii','ignore') #not used for now
        ...
        if not os.path.exists("/mnt/resultimages"):
            os.makedirs("/mnt/resultimages")

        output_path =  "/mnt/resultimages/" + file_name + "_binarize_simpletresh_" + str(threshold_value) + file_extension
