...
- (void)writeSymbolName:(CPString)newName
{
    ...
    if (! bin_already_exists)
    {
        ...
        [symbolCollectionArrayController bind:@"content" toObject:theClassifier withKeyPath:@"symbolCollections" options:nil];  // doesn't actually need to be bound yet
    ...
    for (var i = 0; i < cvArrayControllers_count; ++i)
    {
        ...
        while ([[cvArrayControllers[i] selectedObjects] count] > 0)
        {
            ...
            var glyph = [[theClassifier symbolCollections][i] glyphList][[[cvArrayControllers[i] selectionIndexes] firstIndex]];
            ...
            [[theClassifier symbolCollections][i] removeGlyph:glyph];

            if ([[cvArrayControllers[i] selectedObjects] count] === 0)
            {
                [cvArrayControllers[i] bind:@"contentArray" toObject:[theClassifier symbolCollections][i] withKeyPath:@"glyphList" options:nil];
            }

            [glyph writeSymbolName:newName];
            [[theClassifier symbolCollections][newBinIndex] addGlyph:glyph];
